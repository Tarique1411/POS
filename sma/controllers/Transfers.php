...
class Transfers extends MY_Controller
{
    ...
    function index()
    {
        //***** Added By Anil 21-09-2016 start****        
            $arr_trnsfr = $this->site->checkPermissions();  
            if($arr_trnsfr[0]['transfers-list'] == NULL && (! $this->Owner)){
...
//    </div></div>';
        ...
        /* Permission for action Modified By Anil 17-09-2016 Start */
        if(!$this->Owner) {
        $arr_prmsn =  $this->site->checkPermissions(); 
            if($arr_prmsn[0]['transfers-details'] != 1) {
                $detail_link = '';
            }
            if($arr_prmsn[0]['transfers-edit'] != 1) {
                $edit_link = '';
            }           
            if($arr_prmsn[0]['transfers-pdf'] != 1) {
                $pdf_link = '';
            }
            if($arr_prmsn[0]['transfers-email'] != 1) {
                $email_link = '';
            }
            if($arr_prmsn[0]['transfers-delete'] != 1) {
                $delete_link = '';
            }
        }    
       ...
       /* Permission for action Modified By Anil 14-09-2016 End */   
    ...
    function add()
    {
        ...
        //if ($this->form_validation->run()) {
          ...
          //  }
            ...
            for ($r = 0; $r < $i; $r++) { 
                ...
                $invoice_no = $_POST['invno'][$r];
                $invoice_date = $_POST['invdate'][$r];
                $product_id = $_POST['product_id'][$r];
                $product_name = $_POST['product_name'][$r];
                $quantity = $_POST['quantity'][$r];
                $warehouse_id = $this->session->userdata('warehouse_id');
                $product_hsn = $_POST['product_hsn'][$r];
                $product_type = $_POST['product_type'][$r];
                $lot_no = $_POST['product_lot'][$r];
               ...
               // }
            } //echo "<pre>"; print_r($data); echo "<br><pre>"; print_r($products); die;
            if (empty($products)) {
       ...
       // if ($this->form_validation->run() == true && $this->transfers_model->addTransfer($data, $products)) {
        if ($_POST['quantity'] && $this->transfers_model->updateQty($data, $products)) {
            $this->session->set_userdata('remove_tols', 1);
            $this->session->set_flashdata('message', lang("Purchase Return Edit Successfully"));
            redirect("welcome");
        } else {
            ...
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('transfers'), 'page' => lang('Purchase Return')), array('link' => '#', 'page' => lang('Add Purchase Return')));
    ...
    function transfer_by_csv()
    {
         //***** Added By Anil 16-08-2016 start****        
            $arr_trnsfr_csv = $this->site->checkPermissions();  
            if($arr_trnsfr_csv[0]['transfers-add_by_csv'] == NULL && (! $this->Owner)){
                $this->session->set_flashdata('error', lang("access_denied"));
                redirect('welcome'); 
            }
        ...
        if ($rows) {
            foreach ($rows as $row) {
                ...
                if ($row->tax_rate) {
                    ...
                    $pr[] = array('id' => str_replace(".", "", microtime(true)), 'item_id' => $row->id, 'label' => " (" . $row->code . ") invoice_no- " . $row->inv_no . ", invoice_date- (". date("d-m-Y", strtotime( $row->inv_date)) .")", 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'options' => $options);
                } else {
                    $pr[] = array('id' => str_replace(".", "", microtime(true)), 'item_id' => $row->id, 'label' => " (" . $row->code . ") invoice_no- " . $row->inv_no . ", invoice_date- (". date("d-m-Y", strtotime( $row->inv_date)) .")", 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => false, 'options' => $options);
                }
            }
            echo json_encode($pr);
        } else {

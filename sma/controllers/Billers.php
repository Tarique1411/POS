...
class Billers extends MY_Controller
{
    ...
    function __construct()
    {
        ...
        if (!$this->Owner && !$this->Admin) { // modified By Anil 
    ...
    function index($action = NULL)
    {
        //***** Added By Anil 16-08-2016 start****             
            $arr_billers = $this->site->checkPermissions();  
            if($arr_billers[0]['billers-list'] == NULL && (! $this->Owner)){
    ...
    /* ***** Delete Permissions 
     *  Modified By Anil Start *****
     */
        if($this->Owner) {
        $delete_link = "<a href='#' class='tip po' title='<b>" . $this->lang->line("delete_biller") . "</b>' data-content=\"<p>" . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . site_url('billers/delete/$1') . "'>" . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i></a>";
        } else {
            $arr = $this->site->checkPermissions();  
            if($arr[0]['billers-delete']){
            $delete_link = "<a href='#' class='tip po' title='<b>" . $this->lang->line("delete_biller") . "</b>' data-content=\"<p>" . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . site_url('billers/delete/$1') . "'>" . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i></a>";
            }
        }    
        $this->datatables
            ...
            ->add_column("Actions", "<center><a class=\"tip\" title='" . $this->lang->line("edit_biller") . "' href='" . site_url('billers/edit/$1') . "' data-toggle='modal' data-target='#myModal'><i class=\"fa fa-edit\"></i></a>" .$delete_link. "</center>", "id");
        //->unset_column('id');
     /*  ***** Delete Permissions 
     *  Modified By Anil End *****
     */    
    ...
    {
        //***** Added By Anil 16-08-2016 start****             
            $arr_billers_add = $this->site->checkPermissions();  
            if($arr_billers_add[0]['billers-add'] == NULL && (! $this->Owner)){
                $this->session->set_flashdata('error', lang("access_denied"));
                redirect('welcome'); 
            }
        //***** Added By Anil 16-08-2016 End**** 

...
class Quotes extends MY_Controller
{
    ...
    function index($warehouse_id = NULL)
    {
        //***** Added By Anil 21-09-2016 start****        
            $arr_quote_list = $this->site->checkPermissions();  
            if($arr_quote_list[0]['quotes-list'] == NULL && (! $this->Owner)){
...
//                </div></div>';
        ...
        /* Permission for action added By Anil 17-09-2016 Start */
        if(!$this->Owner) {
        $arr_prmsn =  $this->site->checkPermissions(); 
            if($arr_prmsn[0]['quotes-details'] != 1) {
                $detail_link = '';
            }
            if($arr_prmsn[0]['quotes-edit'] != 1) {
                $edit_link = '';
            }
            if($arr_prmsn[0]['sales-add'] != 1) {
                $convert_link = '';
            }
            if($arr_prmsn[0]['purchases-add'] != 1) {
                $pc_link = '';
            }
            if($arr_prmsn[0]['quotes-pdf'] != 1) {
                $pdf_link = '';
            }
            if($arr_prmsn[0]['quotes-email'] != 1) {
                $email_link = '';
            }
            if($arr_prmsn[0]['quotes-delete'] != 1) {
                $delete_link = '';
            }
        }    
    ...
    {
        //***** Added By Anil 21-09-2016 start****        
            $arr_quote_add = $this->site->checkPermissions();  
            if($arr_quote_add[0]['quotes-add'] == NULL && (! $this->Owner)){
                $this->session->set_flashdata('error', lang("access_denied"));
                redirect('welcome'); 
            }
        //***** Added By Anil 21-09-2016 End**** 
        ...
        // ******* Added By Anil 22-09-2016 Start *******    
            if($this->Owner) {
                ...
                $this->data['billers'] = $this->Admin ? $this->site->getAllCompanies('biller') : NULL;
                $this->data['warehouses'] = $this->Admin ? $this->site->getAllWarehouses() : NULL;
            }
    ...
    {
        //***** Added By Anil 21-09-2016 start****        
            $arr_quote_edit = $this->site->checkPermissions();  
            if($arr_quote_edit[0]['quotes-edit'] == NULL && (! $this->Owner)){
                $this->session->set_flashdata('error', lang("access_denied"));
                redirect('welcome'); 
            }
        //***** Added By Anil 21-09-2016 End**** 

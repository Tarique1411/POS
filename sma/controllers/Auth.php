...
class Auth extends MY_Controller
{
    ...
    function users()
    {
        //***** Added By Anil 20-09-2016 start****       
            ...
            if($arr_list[0]['users_list'] == NULL && (! $this->Owner)){
        ...
        if ( !($this->Manager || $this->Admin || $this->Owner)) { 
       /* ***Modified By Anil 20-09-2016*** */ 
    ...
    function getUsers()
    {
        ...
        /*  ***Comments By Anil Start   *****
		if($this->session->all_userdata()['biller_id'] != null){
			$this->datatables
				//->select($this->db->dbprefix('users').".id as id, first_name, last_name, email, company, award_points, " . $this->db->dbprefix('groups') . ".name, active")
                                ->select($this->db->dbprefix('users').".id as id, first_name, last_name, warehouse_id, " . $this->db->dbprefix('groups') . ".name, active")
				->from("users")
				->join('groups', 'users.group_id=groups.id')
				->group_by('users.id')
				->where('company_id', NULL)
				->where('biller_id',$this->session->all_userdata()['biller_id'])  
...
//                                if($this->Owner){
//				$this->datatables->add_column("Actions", "<center><a href='" . site_url('auth/profile/$1') . "' class='tip' title='" . lang("edit_user") . "'><i class=\"fa fa-edit\"></i></a></center>", "id");
//                                } 
            ...
            ***Comments By Anil End ****
        */
    /* ***Added By Anil for get Users 
         * List Based On Users Loing Start  */
        $this->datatables->select($this->db->dbprefix('users').".id as id,first_name, last_name, "
                . $this->db->dbprefix('warehouses').".name as wname, " . $this->db->dbprefix('groups') . ".name, active");
        $this->datatables->from("users");
        $this->datatables->join('groups', 'users.group_id=groups.id');
        $this->datatables->join('warehouses', 'users.warehouse_id=warehouses.id');
        $this->datatables->group_by('users.id');
        $this->datatables->where('company_id', NULL);
        if($this->Owner) {           
            $this->datatables->where('group_id !=',1);
         } else if($this->Admin) { 
            $this->datatables->where('group_id !=',1); 
            $this->datatables->where('group_id !=',$this->session->userdata('group_id')); 
         } else {            
            $this->datatables->where('group_id !=',1); 
            $this->datatables->where('groups.name !=','admin');
            $this->datatables->where('group_id !=',$this->session->userdata('group_id')); 
            ...
            if($this->Admin || $this->Owner){
                $this->datatables->add_column("Actions", "<center><a href='" . site_url('auth/profile/$1') . "' "
                  . "class='tip' title='" . lang("edit_user") . "'><i class=\"fa fa-edit\"></i></a></center>","id");
            }else{
                $this->datatables->add_column("Actions", "");
            }
      
        /* ***Added By Anil for get Users 
    * List Based On Users Loing End  */
    ...
    {   
        //**** Group name Modified by Anil ****  
        if (!$this->ion_auth->logged_in() || !$this->ion_auth->in_group('owner','admin','manager') && $id != $this->session->userdata('user_id')) {
            $this->session->set_flashdata('warning', lang("access_denied"));
            redirect($_SERVER["HTTP_REFERER"]);
        }
        if (!$id || empty($id)) {
    ...
    function login($m = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
           ...
           // $store_id= $this->input->post('select_store'); //----- Get store id add @ Ankit
           //  $group_id= $this->input->post('select_group'); //----- Get group id add @ Ankit
           //echo "store=>".$select_store." ==group=>".$select_group; die();
            ...
            $this->data['select_store'] = $this->auth_model->getAllStore();
            $this->data['select_group'] = $this->auth_model->getAllGroup();
    ...
    /* Updated by ajay for open modal for activate user
        on 20-01-2017
    */
//    function bactivate($id, $code = false)
//    {
//        if ($code !== false) {
//            $activation = $this->ion_auth->activate($id, $code);
//        } else if ($this->Owner || $this->Admin || $this->Manager) {   //**** Groups Edited By Anil **** 
//            $activation = $this->ion_auth->activate($id);
//        }      
//
//        if ($activation) {
//            $this->session->set_flashdata('message', $this->ion_auth->messages());
//            if ($this->Owner || $this->Admin || $this->Manager) {  //**** Groups Edited By Anil **** 
//                redirect($_SERVER["HTTP_REFERER"]);
//            } else {
//                redirect("auth/login");
//            }
...
//    }
    ...
    function activate($id, $code = false){
        ...
        if ($this->form_validation->run() == FALSE) {
            if ($this->input->post('activate')) {
                ...
                $this->load->view($this->theme . 'auth/activate_user', $this->data);
            }
        } else {
            ...
            if ($this->input->post('confirm') == 'yes') {
                ...
                //**** Groups Edited By Anil ****
                if ($this->ion_auth->logged_in() && ($this->Owner || $this->Admin || $this->Manager)) {                 
    ...
    function create_user()
    {
        //***** Added By Anil 20-09-2016 start****       
            ...
            if($arr_add[0]['users-add'] == NULL && (! $this->Owner)){
                $this->session->set_flashdata('error', lang("access_denied"));
                redirect('welcome'); 
            }
        //***** Added By Anil 20-09-2016 End****       
            
        if (!$this->Owner && ! $this->Admin && ! $this->Manager) {  /* Modified By Anil  */
            $this->session->set_flashdata('warning', lang("access_denied"));
            redirect($_SERVER["HTTP_REFERER"]);
        }
        ...
        if ($this->form_validation->run() == true) {
            ...
            $username = strtolower($this->addCharacterUsername());
    ...
    function edit_user($id = NULL)
    {
        ...
        if ($this->Admin) {
            if($this->input->post('update') == 'Update'){
                ...
                //echo "<pre>";$this->sma->print_arrays($data,$this->input->post());
                
                if($this->auth_model->updateGroup($user->id, $data)){
...
//        if ($this->form_validation->run() === TRUE) {
//
//            /* Modified By Anil 08-09-2016 Start */
...
//        }
    ...
    function addCharacterUsername()
    {
        $biller_id = $this->input->post('biller');
        $username = $this->input->post('username');
        $this->db->select("id,company,city");
        ...
        if ($q->num_rows() > 0) {
            ...
            $str1= substr($data[0]->company, 0, 1);
            $str2= substr($data[0]->city, 0, 2);
            $user_name= $str1.$str2.$username;
            return $user_name;

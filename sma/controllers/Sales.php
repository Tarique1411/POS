...
class Sales extends MY_Controller {
    ...
    function __construct() {
        ...
        $this->load->model("msync_model"); //Updated by Ankit
    ...
    function index($warehouse_id = NULL) {
        ...
        $arr_list = $this->site->checkPermissions();
        if ($arr_list[0]['sales-list'] == NULL && (!$this->Owner)) {
    ...
    function getSales($warehouse_id = NULL) {
        ...
        if ((!$this->Owner || !$this->Admin || !$this->Manager) && !$warehouse_id) {
        ...
        if (!$this->Owner) {
            ...
            if ($arr_prmsn[0]['sales-sales_details'] != 1) {
                $detail_link = '';
            }
            if ($arr_prmsn[0]['sales-view_payments'] != 1) {
                $payments_link = '';
            }
            if ($arr_prmsn[0]['sales-payments'] != 1) {
                $add_payment_link = '';
            }
            if ($arr_prmsn[0]['sales-add_delivery'] != 1) {
                $add_delivery_link = '';
            }
            if ($arr_prmsn[0]['sales-edit'] != 1) {
                $edit_link = '';
            }
            if ($arr_prmsn[0]['sales-pdf'] != 1) {
                $pdf_link = '';
            }
            if ($arr_prmsn[0]['sales-email'] != 1) {
                $email_link = '';
            }
            if ($arr_prmsn[0]['sales-return_sales'] != 1) {
                $return_link = '';
            }
            if ($arr_prmsn[0]['sales-delete'] != 1) {
                $delete_link = '';
            }
        }
    ...
    function return_sales($warehouse_id = NULL) {
        //***** Added By Anil 21-09-2016 start****        
        $arr_return_sales = $this->site->checkPermissions();
        if ($arr_return_sales[0]['sales-return_sales'] == NULL && (!$this->Owner)) {
            $this->session->set_flashdata('error', lang("access_denied"));
            redirect('welcome');
        }
        //***** Added By Anil 21-09-2016 End**** 
        ...
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        if ($this->Owner || $this->Admin) {     // Admin added By Anil
            $this->data['warehouses'] = $this->site->getAllWarehouses();
            $this->data['warehouse_id'] = $warehouse_id;
            $this->data['warehouse'] = $warehouse_id ? $this->site->getWarehouseByID($warehouse_id) : NULL;
        } else {
    ...
    function getReturns($warehouse_id = NULL) {
        ...
        if (!$this->Owner && !$this->Admin && !$warehouse_id) {  // Admin added By Anil
            $user = $this->site->getUser();
            $warehouse_id = $user->warehouse_id;
        }
        ...
        if ($warehouse_id && $chek == false) {
            $this->datatables
                    ->select($this->db->dbprefix('return_sales') . ".id as id, "
                            . $this->db->dbprefix('return_sales') . ".date as date, "
                            . $this->db->dbprefix('return_sales') . ".reference_no as ref, "
                            . $this->db->dbprefix('return_sales') . ".sales_reference_no as sal_ref, "
                            . " CASE WHEN sma_sales.return_flg = 1 THEN 'Returned' "
                            . " WHEN sma_sales.return_flg = 2 THEN 'Partial Return' "
                            . " WHEN sma_sales.return_flg = 0 THEN 'Sold' "
                    ...
                    ->join('companies', 'companies.id = return_sales.customer_id', 'left')
            ...
            $this->db->order_by('return_sales.id', 'desc');
    ...
    function view_return($id = NULL) {
        ...
        $this->data['warehouse'] = $this->site->getWarehouseByID($inv->warehouse_id);
        $credit_note = $this->sales_model->getGiftCards($inv->customer_id, $inv->id, $inv->sale_id);
        $this->data['credit_note'] = $credit_note;
        $this->data['redemption_details'] = $this->sales_model->getRedemptionDetails($credit_note->card);//added by rana 4th jan 2018
        ...
        if ($this->form_validation->run() == true) {
            ...
            if ($skrill->active == "1" && $inv->grand_total != "0.00") {
                ...
                $btn_code .= ' <a href="https://www.moneybookers.com/app/payment.pl?method=get&pay_to_email='
                        . $skrill->account_email . '&language=EN&merchant_fields=item_name,item_number&item_name='
                        . $inv->reference_no . '&item_number=' . $inv->id
                        . '&logo_url=' . base_url() . 'assets/uploads/logos/' . $this->Settings->logo
                        . '&amount=' . (($inv->grand_total - $inv->paid) + $skrill_fee) . '&return_url='
                        . site_url('sales/view/' . $inv->id) . '&cancel_url=' . site_url('sales/view/' . $inv->id)
                        . '&detail1_description=' . $inv->reference_no . '&detail1_text=Payment for the sale invoice '
                        . $inv->reference_no . ': ' . $inv->grand_total . '(+ fee: ' . $skrill_fee . ') = '
                        . $this->sma->formatMoney($inv->grand_total + $skrill_fee) . '&currency='
                        . $this->default_currency->code . '&status_url=' . site_url('payments/skrillipn')
                        . '"><img src="' . base_url('assets/images/btn-skrill.png') . '" alt="Pay by Skrill"></a>';
            }
    ...
    function add($quote_id = NULL) {
        ...
        if ($arr_add[0]['sales-add'] == NULL && (!$this->Owner)) {
            $this->session->set_flashdata('error', lang("access_denied"));
            redirect('welcome');
        }
        //***** Added By Anil 21-09-2016 End**** 
    ...
    function check_ret_sale($id = NULL) {
        $this->sma->checkPermissions('return_sales');
        if ($this->input->post('id')) {
            $id = $this->input->post('id');
        }
        //echo $id;
        $res = $this->sales_model->getReturnDate($id);
        $sale_date = date("Y-m-d", strtotime($res->date));
        $exp_date = date("Y-m-d", strtotime('+' . $this->pos_settings->return_sales_perioed . ' days' . $sale_date));
        $cur_date = date("Y-m-d");
    ...
    function return_sale($id = NULL) {
        ...
        $this->data['possettings'] = $this->pos_model->getSetting();
        //print_r($this->data['possettings']);die;
        // $this->form_validation->set_rules('reference_no', lang("reference_no"), 'required');
        $this->form_validation->set_rules('paid_by', lang("paying_by"), 'required');
        if ($this->input->post('paid_by')) {
            $this->form_validation->set_rules('gift_card_no', 'Credit Voucher no', 'trim|required');
        }
        $this->form_validation->set_rules('return_reason', 'Sales Return Reason', 'trim|required');
        ...
        if ($this->form_validation->run() == true) {
            ...
            $gcData = $this->input->post('gift_card_data');
            $gcData[3] = date('Y-m-d', strtotime($gcData[3]));
            ...
            $sales_reference = $this->input->post('reference_no');
            if ($this->Owner || $this->Admin) {
                $date = $this->sma->fld(trim($this->input->post('date')));
            } else {
                $date = date('Y-m-d H:i:s');
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                $sale_item_ids = $_POST['sale_item_ids'][$r];
                $item_option = isset($_POST['product_option'][$r]) && $_POST['product_option'][$r] != 'false' ? $_POST['product_option'][$r] : NULL;
                //$option_details = $this->sales_model->getProductOptionByID($item_option);
                $real_unit_price = $this->sma->formatDecimal($_POST['real_unit_price'][$r]);
                $unit_price = $this->sma->formatDecimal($_POST['unit_price'][$r]);
                ...
                if (isset($item_code) && isset($real_unit_price) && isset($unit_price) && isset($item_quantity)) {
                    ...
                    if (isset($item_tax_rate) && $item_tax_rate != 0) {
                        ...
                        /* if ($product_details && $product_details->tax_percentage) {
                        ...
                        /* } elseif ($tax_details->type == 2) {
                          ...
                          if ($product_details && $product_details->tax_percentage == 1) {
                          $item_tax = $this->sma->formatDecimal((($unit_price) * $tax_details->rate) / 100);
                          $tax = $tax_details->rate . "%";
                          } else {
                          $item_tax = $this->sma->formatDecimal((($unit_price) * $tax_details->rate) / (100 + $tax_details->rate));
                    ...
                    $product_tax += $pr_item_tax;
                    $subtotal = (($item_net_price * $item_quantity) + $pr_item_tax);
                    // echo "product tax".$product_tax;exit;
                    $products[] = array(
                        'sale_id' => $id,
                        'product_id' => $item_id,
                        'product_code' => $item_code,
                ...
                /* if (isset($item_code) && isset($real_unit_price) && isset($unit_price) && isset($item_quantity)) {
                  ...
                  if ($dpos !== false) {
                  $pds = explode("%", $discount);
                  $pr_discount = (($this->sma->formatDecimal($net_price)) * (Float)($pds[0])) / 100;
                  } else {
                  $pr_discount = $this->sma->formatDecimal($discount);
                  }
                  } else {
                  ...
                  $unit_price = $this->sma->formatDecimal($net_price - $pr_discount);
                  $pr_item_discount = $this->sma->formatDecimal($pr_discount * $item_quantity);
                  $product_discount += $pr_item_discount;
                  ...
                  $item_net_price = $product_details->tax_method ? $this->sma->formatDecimal($unit_price) : $this->sma->formatDecimal($unit_price-$item_tax);
                  $product_tax += $pr_item_tax;
                  $subtotal = (($item_net_price * $item_quantity) + $pr_item_tax);
            ...
            if ($this->input->post('discount')) {
                $order_discount_id = $this->input->post('order_disc');
                //echo '<br>disc --- '.$this->input->post('rediscount');
                /* $opos = strpos($order_discount_id, $percentage);
                  if ($opos !== false) {
                  $ods = explode("%", $order_discount_id);
                  $order_discount = $this->sma->formatDecimal((($total + $product_tax) * (Float)($ods[0])) / 100);
                ...
                if ($order_discount_id == NULL) {
                    if ($this->pos_settings->order_discount_type == 'percent') {
                        //$pds = $discount;
                        $order_discount = (($this->sma->formatDecimal($total + $product_tax)) * (Float) ($order_discount_id)) / 100;
                    } else {
                        $order_discount = $this->sma->formatDecimal($order_discount_id);
                    }
                } else {
            ...
            $data = array('date' => $date,
                ...
                'grand_total' => $grand_total,
                'created_by' => $this->session->userdata('user_id'),
                'return_time' => $this->pos_settings->cv_expiry,
            ...
            if ($this->input->post('amount-paid') && $this->input->post('amount-paid') != 0) {
                $payment = array(
                    ...
                    'paid_by' => $this->input->post('paid_by'),
                    'cheque_no' => $this->input->post('cheque_no'),
                    //'gift_card_balance' => $this->input->post('cv_balance'),
                    //'gift_card_value' => $this->input->post('cv_value'),
                    'cc_no' => $this->input->post('pcc_no'),
                    'cc_holder' => $this->input->post('pcc_holder'),
                    'cc_month' => $this->input->post('pcc_month'),
                    'cc_year' => $this->input->post('pcc_year'),
        ...
        }//echo "Payment ---- <pre>";print_r($gcData);die;
        if ($this->form_validation->run() == true) {
            //echo "<pre>";print_r($gcData);die;
            $return_id = $this->sales_model->returnSale($data, $products, $payment, $gcData);
            if (isset($return_id) && ($return_id > 0)) {
                $gift_card_update = $this->sales_model->updateCVBalanceReturnSale($payment['gift_card_no'], $payment['gift_card_balance'], $payment['gift_card_value'], $data['grand_total']);
                $this->session->set_flashdata('message', lang("return_sale_added"));
                redirect("sales/view_return/" . $return_id);
            }
        } else {
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['inv'] = $this->sales_model->getInvoiceByID($id);
            ...
            $inv_items = $this->sales_model->getAllInvoiceItemsRemaining($id);
            if (count($inv_items) == 0) {
                // redirect("welcome");
                $this->data['error'] = $this->session->set_flashdata('message', lang('all_item_returned'));
            }
            $c = rand(100000, 9999999);
            foreach ($inv_items as $item) {
                $row = $this->site->getProductByID($item->product_id);
            ...
            $this->data['inv_items'] = json_encode($pr);
            $this->data['return_reasons'] = $this->sales_model->getSalesReturnReasons();
            $this->data['sales'] = $this->sales_model->getInvoiceByID($id);
            $this->data['id'] = $id;
            $this->data['payment_ref'] = $this->site->getReference('pay');
            $this->data['settings'] = $this->sales_model->getCVSetting();
            $this->data['reference'] = $this->data['sales']->reference_no;
            $this->data['tax_rates'] = $this->site->getAllTaxRates();
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
                ...
                if ($this->input->post('form_action') == 'export_excel' || $this->input->post('form_action') == 'export_pdf') {
                    $biller_details = $this->site->getCompanyByID($_SESSION['biller_id']);
                    $this->load->library('excel');
                    $this->excel->setActiveSheetIndex(0);
                    $style = array(
                        'fill' => array(
                            'type' => PHPExcel_Style_Fill::FILL_SOLID,
                            'color' => array('rgb' => 'E75CC2')
                        ),
                        'font' => array(
                            'name' => 'Verdana',
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('A1', $biller_details->name . " Sales");
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('E2', lang('grand_total'));
                    $this->excel->getActiveSheet()->SetCellValue('F2', lang('paid'));
                    ...
                    foreach ($_POST['val'] as $id) {
                        $sale = $this->sales_model->getInvoiceByID($id);
                        if ($sale->return_flg == '1') {
                            $paid = 'Returned';
                        } elseif ($sale->return_flg == '2') {
                            $paid = 'Partial Return';
                        } else {
                            $paid = 'Paid';
                        }
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('E' . $row, $this->default_currency->code . ' ' . $this->sma->formatMoney($sale->grand_total));
                        $this->excel->getActiveSheet()->SetCellValue('F' . $row, $this->default_currency->code . ' ' . $this->sma->formatMoney($sale->paid));
                        $this->excel->getActiveSheet()->SetCellValue('G' . $row, ucwords($sale->payment_status));
    ...
    function deliveries() {
        //***** Added By Anil 20-08-2016 start****        
        $arr_deliveries = $this->site->checkPermissions();
        if ($arr_deliveries[0]['sales-deliveries'] == NULL && (!$this->Owner)) {
            $this->session->set_flashdata('error', lang("access_denied"));
            redirect('welcome');
        }
        //***** Added By Anil 20-08-2016 End**** 
    ...
    function getDeliveries() {
        ...
        if (!$this->Owner) {
            $arr_prmsn = $this->site->checkPermissions();
            if ($arr_prmsn[0]['sales-details_delivery'] != 1) {
                $detail_link = '';
            }
            if ($arr_prmsn[0]['sales-edit_delivery'] != 1) {
                $edit_link = '';
            }
            if ($arr_prmsn[0]['sales-pdf_delivery'] != 1) {
                $pdf_link = '';
            }
            if ($arr_prmsn[0]['sales-delete_delivery'] != 1) {
                $delete_link = '';
            }
        }
        /* Permission for action added By Anil 21-09-2016 End */
    ...
    function updatecart() {
        $item_id1 = $this->input->get('item_id1', TRUE);
        $quantity1 = $this->input->get('quantity1', TRUE);
        $cart_count1 = $this->input->get('cart_count1', TRUE);
        $final_cart_count1 = $cart_count1 + 1;
        $final_count = $quantity1 - $final_cart_count1;

        if ($final_count == 0) {
            $this->db->query("update sma_products set cart_qty = 0, cart_count =" . $final_cart_count1 . " where id = " . $item_id1 . " ");
            return 1;
        } else if ($final_count > 0) {
            $this->db->query("update sma_products set cart_count =" . $final_cart_count1 . " where id = " . $item_id1 . " ");
            return 1;
        } else {
            $this->db->query("update sma_products set cart_qty = 0 where id = " . $item_id1 . " ");
            return 1;
        }
    }

    function updatecart_del() {
        $item_id2 = $this->input->get('item_id2', TRUE);
        $this->db->select("cart_count");
        $this->db->from("sma_products");
        $this->db->where("id", $item_id2);
        $q = $this->db->get();
        $cart_count3 = $q->result();
        $final_count = $cart_count3[0]->cart_count;
        $updated_count = $final_count - 1;
        if ($final_count > 0) {
            if ($this->db->query("update sma_products set cart_qty =1, cart_count =" . $updated_count . " where id = " . $item_id2 . " ")) {
                return true;
            }
        }
    }
    ...
    function suggestions() {
        ...
        if ($rows) {
            ...
            foreach ($rows as $row) {
                ...
                $option = FALSE;
                $row->quantity = 1;
                $row->item_tax_method = $row->tax_rate;
                //echo "<pre>";print_r($row->item_tax_method);die;
                $row->qty = 1;
                $row->discount = '0';
                $row->discount_type = '';
                $row->serial = '';
                ...
                if ($row->tax_rate) {
                    ...
                    $pr[] = array('id' => str_replace(".", "", microtime(true)), 'discount_type' => $pos_settings->discount_type, 'overselling' => $settings->overselling, 'item_id' => $row->id, 'label' => $row->code, 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'options' => $options);
                } else {
                    $pr[] = array('id' => str_replace(".", "", microtime(true)), 'discount_type' => $pos_settings->discount_type, 'overselling' => $settings->overselling, 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")" . " - " . $this->default_currency->code . " " . $row->price, 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => false, 'options' => $options);
                }
            }
            //print_r($pr); die;
            echo json_encode($pr);
    ...
    //Added by Chitra on 07/09 for lot changes

    public function productslot() {
        ...
        $lot = $this->sales_model->getProductsLot($prid, $code, $warehouse_id);
        //echo "<pre>";print_r($lot);
...
//            foreach ($rows as $row) {
//                $code = $row->code;
//                $lot = $this->sales_model->getProductsLot($code, $warehouse_id);
//            }
//            echo json_encode($lot) ;
//        }
    }
    ...
    function POSCVdataSync() {
        //echo $_SESSION['org_id'];die;
        if ($this->msync_model->creditVoucherMaster_final($_SESSION['org_id'])) {
            if ($this->msync_model->Credit_sync_status()) {
                $this->session->set_flashdata('message', "Credit Voucher Data Sync Successfully");
                redirect('sales/credit_note');
            }
        }
    }

    function intermediateCVdataSync() {
        $invoice_no = $this->input->post('name');
        //echo $invoice_no."<br>"; die;
        if(!$this->msync_model->checkInvoiceEoId($invoice_no)){
            echo 'EO_ID'; exit;
        }
        if ($this->msync_model->cvMaster_final($invoice_no)) {
            $cid = 1;
    ...
    function credit_note() {
        //***** Added By Anil 21-09-2016 start****        
        $arr_gift_cards = $this->site->checkPermissions();
        //print_r($arr_gift_cards);exit;
        if ($arr_gift_cards[0]['sales-gift_cards'] == NULL && (!$this->Owner)) {
            $this->session->set_flashdata('error', lang("access_denied"));
            redirect('welcome');
        }
        //***** Added By Anil 21-09-2016 End**** 
    ...
    function getGiftCards() {
        ...
        $this->datatables
                ...
                ->from("gift_cards")
                ->add_column("Actions", "<center><a href='" . site_url('sales/view_gift_card/$1') . "' "
                        . "class='tip' title='" . lang("view_gift_card") . "' data-toggle='modal' "
                        . "data-target='#myModal'><i class=\"fa fa-eye\"></i></a> </center>", "id");
...
//->unset_column('id');
    ...
    function view_gift_card($id = NULL) {
        ...
        $credit_note = $this->data['gift_card'];
        $inv = $this->sales_model->getReturnSalesByID($credit_note->return_id);
        $this->data['inv'] = $inv;
        //$this->data['credit_note_val'] = $this->sales_model->getGiftCards($inv->customer_id, $credit_note->return_id, $inv->card_no);
        $this->data['credit_note_val'] = $credit_note;
        $this->data['customer'] = $this->site->getCompanyByID($inv->customer_id);
        $this->data['payments'] = $this->sales_model->getPaymentsForSale($credit_note->sales_id);
        $this->data['biller'] = $this->site->getCompanyByID($inv->biller_id);
        $this->data['pos'] = $this->pos_model->getSetting();
        $this->data['pos_settings'] = $this->pos_settings;
        $this->data['user'] = $this->site->getUser($inv->created_by);
        $this->data['warehouse'] = $this->site->getWarehouseByID($inv->warehouse_id);
        $this->data['rows'] = $this->sales_model->getAllReturnItems($credit_note->return_id);
        $this->data['controller'] = "1";
        $credit_note_details = $this->sales_model->getGiftCards($inv->customer_id, $inv->id, $inv->sale_id);//added by rana 4th jan 2018
        $this->data['redemption_details'] = $this->sales_model->getRedemptionDetails($credit_note_details->card);//added by rana to  fetch redemption details
        $this->data['sale'] = $this->sales_model->getInvoiceByID($inv->sale_id);
        $bc = array(array('link' => base_url(), 'page' => lang('home')),
            array('link' => site_url('sales'), 'page' => lang('sales')),
            array('link' => '#', 'page' => lang('credit_note')));
        $meta = array('page_title' => lang('credit_note'), 'bc' => $bc);
        //$this->page_construct('sales/view_gift_card', $meta, $this->data);
//        dd($this->data);
     ...
     * Added By Ajay
     * on 15-03-2017
     * To generate pdf of credit note
     */

    public function giftCardPdf($id) {
        if ($this->input->get('cn_id')) {
            $id = $this->input->get('cn_id');
        }
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $this->data['gift_card'] = $this->site->getGiftCardByID($id);
        $credit_note = $this->data['gift_card'];
        ...
        $meta = array('page_title' => lang('credit_note'), 'bc' => $bc);
        $name = lang("sale") . "_" . str_replace('/', '_', $inv->reference_no) . ".pdf";
        $html = $this->load->view($this->theme . 'sales/credit_note_pdf', $this->data, TRUE);
        if ($view) {
            $this->load->view($this->theme . 'sales/credit_note_pdf', $this->data);
        } else {
            $this->sma->generate_pdf($html, $name, FALSE, $this->data['biller']->invoice_footer);
        }
    }
    ...
    function validate_gift_card() {
        //$this->sma->checkPermissions();
        $cv_setting = $this->sales_model->getCVSetting();
        $no = $this->input->post('cn');
        $amount = $this->input->post('amount');
        $customer = $this->input->post('customer_id');
        $gc = $this->site->getGiftCardByNO($no);

        if ($gc = $this->site->getGiftCardByNO($no)) {
            $date = strtotime("+" . $cv_setting->cv_grace_period . " days", strtotime($gc->expiry));
            $gc->expiry = date("Y-m-d", $date);

            if ($gc->expiry) {
                if (strtotime($gc->expiry) < strtotime(date('Y-m-d'))) {
                    $gc->error = 'expired';
                    echo json_encode($gc);
                } else if ($gc->balance < $amount) {
                    $gc->error = 'less_balance';
                    echo json_encode($gc);
                } else if ($gc->customer_id !== $customer) {
                    $gc->error = 'other_customer';
                    echo json_encode($gc);
                } else {
                    $gc->error = 'noerror';
                    echo json_encode($gc);
    ...
    function validate_gift_card_return_sales($card_no, $customer_id) {
        if ($gc = $this->site->getGiftCardByCustomerID($card_no, $customer_id)) {
            if ($gc->expiry) {
    ...
    function add_gift_card() {
        ...
        $this->form_validation->set_rules('value', lang("value"), 'trim|required|numeric');
        ...
        if ($this->form_validation->run() == true) {
            ...
            $customer = $customer_details ? (!empty($customer_details->company) ? $customer_details->company : $customer_details->name) : NULL;
            $card_details = explode('/', $this->input->post('card_no'));
            $data = array('card_no' => $card_details[3],
    ...
    function edit_gift_card($id = NULL) {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $expiry_date = date("Y-m-d", strtotime(str_replace('/', '-', $this->input->post('expiry'))));
    ...
    function sell_gift_card() {
        ...
        //if(!empty($customer_details->company)){
        //$customer = $customer_details ? $customer_details->company : NULL;
        //}else{
        //}
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
                ...
                if ($this->input->post('form_action') == 'export_excel' || $this->input->post('form_action') == 'export_pdf') {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('A1', $biller_details->name . " CREDIT NOTES");
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('D2', lang('expiry'));
                    ...
                    foreach ($_POST['val'] as $id) {
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('A' . $row, $sc->biller_id . '/' . $sc->year . '/CN/' . $sc->card_no);
                        $this->excel->getActiveSheet()->SetCellValue('B' . $row, $this->default_currency->code . ' ' . $this->sma->formatMoney($sc->value));
                        $this->excel->getActiveSheet()->SetCellValue('C' . $row, $this->default_currency->code . ' ' . $this->sma->formatMoney($sc->balance));
                    ...
                    $this->excel->getDefaultStyle()->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
                    $filename = 'credit_notes_' . date('Y_m_d_H_i_s');
                    if ($this->input->post('form_action') == 'export_pdf') {
                        $styleArray = array(
                            'borders' => array(
                                'allborders' => array(
                ...
                $this->session->set_flashdata('error', lang("No_Credit_Voucher_selected"));
                redirect($_SERVER["HTTP_REFERER"]);
            }
        } else {
            $this->session->set_flashdata('error', validation_errors());
    ...
    /*
     * 	Added by Ajay on 03-05-2016
     * 	validate gift card on return sale by customer
     */

    function validateGiftCardReturnSale($card_no = NULL, $customer_id = NULL) {
    ...
    function sale_by_csv() {
        //***** Added By Anil 21-09-2016 start****        
        $arr_sale_by_csv = $this->site->checkPermissions();
        if ($arr_sale_by_csv[0]['add_sale_by_csv'] == NULL && (!$this->Owner)) {
            $this->session->set_flashdata('error', lang("access_denied"));
            redirect('welcome');
        }
        //***** Added By Anil 21-09-2016 End**** 
    ...
    /* Added by Ajay
     * on 16-06-2015
     * add credit Voucher setting
     */

    public function credit_voucher_setting() {
        $this->sma->checkPermissions(false, true);
        $this->form_validation->set_rules('expiry', lang("expiry_in_days"), 'trim|numeric|required');
        $this->form_validation->set_rules('grace_period', lang("grace_period_in_days"), 'trim|required|numeric');

        if ($this->form_validation->run() == true) {
            $data = array('cv_expiry' => $this->input->post('expiry'),
                'cv_grace_period' => $this->input->post('grace_period'),
            );
        } elseif ($this->input->post('credit_voucher_setting')) {
            $this->session->set_flashdata('error', validation_errors());
            redirect("sales/gift_cards");
        }

        if ($this->form_validation->run() == true && $this->sales_model->updateCreditVoucherSetting($data)) {
            $this->session->set_flashdata('message', lang("credit_voucher_setting_updated"));
            redirect("sales/gift_cards");
        } else {
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['modal_js'] = $this->site->modal_js();
            ...
            $this->data['page_title'] = lang("new_gift_card");
            $this->load->view($this->theme . 'sales/credit_voucher_setting', $this->data);
        }
    }

    function focSuggestions() {
        $this->load->model('pos_model');
        $term = $this->input->get('term', TRUE);
        $warehouse_id = $this->input->get('warehouse_id', TRUE);
        $customer_id = $this->input->get('customer_id', TRUE);
        ...
        $customer = $this->site->getCompanyByID($customer_id);
        $customer_group = $this->site->getCustomerGroupByID($customer->customer_group_id);
        $rows = $this->sales_model->getFocProductNames($sr, $warehouse_id);
    ...
    public function checkLatestCNForDate() {
        $latest_cn_no = $this->sales_model->checkLatestCNForDate();
        if ($latest_cn_no) {
            $cn = intval(substr($latest_cn_no->card_no, -3));
            $cn = $cn + 1;
            if (($cn > 0) && ($cn < 10)) {
                $num_padded = sprintf("%03d", $cn);
                $card_no = $_SESSION['biller_id'] . '/' . 'CN' . '/' . date('Ymd') . $num_padded;
                echo json_encode(($card_no));
            } else if (($cn >= 10) && ($cn < 99)) {
                $num_padded = sprintf("%03d", $cn);
                $card_no = $_SESSION['biller_id'] . '/' . 'CN' . '/' . date('Ymd') . $num_padded;
                echo json_encode(($card_no));
            } else if (($cn >= 100) && ($cn < 999)) {
                $num_padded = sprintf("%03d", $cn);
                $card_no = $_SESSION['biller_id'] . '/' . 'CN' . '/' . date('Ymd') . $num_padded;
                echo json_encode(($card_no));
            }
            ...
            $n = '001';
            $card_no = $_SESSION['biller_id'] . '/' . 'CN' . '/' . date('Ymd') . $n;
            echo json_encode(($card_no));
        }
    }

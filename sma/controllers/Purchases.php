...
class Purchases extends MY_Controller
{
    ...
    function index($warehouse_id = NULL)
    {
        //***** Added By Anil 21-09-2016 start****        
            $arr_purchases_list = $this->site->checkPermissions();  
            if($arr_purchases_list[0]['purchases-list'] == NULL && (! $this->Owner)){
...
//        </ul>
//    </div></div>';
    /* Modified By Anil Permissions 
     * for Actions 26-09-2016 */           
        $detail_link = anchor('purchases/view/$1', '<i class="fa fa-file-text-o"></i> ' . lang('purchase_details'));
        $payments_link = anchor('purchases/payments/$1', '<i class="fa fa-money"></i> ' . lang('view_payments'), 'data-toggle="modal" data-target="#myModal"');
        $add_payment_link = anchor('purchases/add_payment/$1', '<i class="fa fa-money"></i> ' . lang('add_payment'), 'data-toggle="modal" data-target="#myModal"');
        $email_link = anchor('purchases/email/$1', '<i class="fa fa-envelope"></i> ' . lang('email_purchase'), 'data-toggle="modal" data-target="#myModal"');
        ...
        $arr_prmsn =  $this->site->checkPermissions(); 
            if($arr_prmsn[0]['purchases-details'] != 1) {
                $detail_link = '';
            }
            if($arr_prmsn[0]['purchases-view_payments'] != 1) {
                $payments_link = '';
            }
            if($arr_prmsn[0]['purchases-payments'] != 1) {
                $add_payment_link = '';
            }           
            if($arr_prmsn[0]['purchases-edit'] != 1) {
                $edit_link = '';
            }
            if($arr_prmsn[0]['purchases-pdf'] != 1) {
                $pdf_link = '';
            }
            if($arr_prmsn[0]['purchases-email'] != 1) {
                $email_link = '';
            }
            if($arr_prmsn[0]['purchases-delete'] != 1) {
                $delete_link = '';    
            }
        }    
...
//generate pdf and force to download
    ...
    {
        //***** Added By Anil 21-09-2016 start****        
            $arr_purchases_add = $this->site->checkPermissions();  
            if($arr_purchases_add[0]['purchases-add'] == NULL && (! $this->Owner)){
                $this->session->set_flashdata('error', lang("access_denied"));
                redirect('welcome'); 
            }
        //***** Added By Anil 21-09-2016 End**** 
    ...
    /* ----------------------------------------------------------------------------------------------------------- */
    /*Updates added by Chitra to add MRN */
    function add_mrn($quote_id = NULL)
    {
        //***** Added By Anil 21-09-2016 start****        
            $arr_purchases_add = $this->site->checkPermissions();  
            if($arr_purchases_add[0]['purchases-add'] == NULL && (! $this->Owner)){
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->addPurchase($data, $products)) {
            ...
            $this->session->set_userdata('user_csrf', $value);
            $this->data['csrf'] = $this->session->userdata('user_csrf');
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('purchases'), 'page' => lang('purchases')), array('link' => '#', 'page' => lang('add_mrn')));
            $meta = array('page_title' => lang('add_mrn'), 'bc' => $bc);
            $this->page_construct('purchases/add_mrn', $meta, $this->data);
        }
    }
    ...
    {
        //***** Added By Anil 21-09-2016 start****        
            $arr_add_csv = $this->site->checkPermissions();  
            if($arr_add_csv[0]['purchase-add_by_csv'] == NULL && (! $this->Owner)){
                $this->session->set_flashdata('error', lang("access_denied"));
                redirect('welcome'); 
            }
        //***** Added By Anil 21-09-2016 End****
    ...
    function expenses($id = NULL)
    {
        //***** Added By Anil 22-09-2016 start****        
            $arr_exp = $this->site->checkPermissions();  
            if($arr_exp[0]['purchases-expenses'] == NULL && (! $this->Owner)){
                $this->session->set_flashdata('error', lang("access_denied"));
                redirect('welcome'); 
            }
        //***** Added By Anil 21-09-2016 End**** 
    ...
    // ***** Functions Modified My Anil  23-09-2016 *****
    function add_expense()
    {
        //***** Added By Anil 22-09-2016 start****     
            $arr_exp_add = $this->site->checkPermissions();
            if($arr_exp_add[0]['pos-tip_addexpense'] == NULL && (! $this->Owner)){
                $this->session->set_flashdata('error', lang("access_denied"));
                redirect('welcome'); 
            }
        //***** Added By Anil 21-09-2016 End**** 
    ...
    public function bankDeposit(){
        if ($this->Owner || $this->Admin) {
            ...
            $data = array(
                'date' => $date,
                'reference' => $this->input->post('slip_no') ? $this->input->post('slip_no') : $this->site->getReference('ex'),
    ...
    /*Function added by Chitra to validate slip no. on bank expenses */
    function getSlip($slip){
        if($slip == NULL){
            $slip = $this->input->post('slip');
        }
        //echo $slip;
            $q = $this->db->get_where('expenses',array('reference'=>$slip));
            if ($q->num_rows() > 0) {
                echo 'yes';
            }
            else{
   ...
   // ***** Functions Modified My Anil  23-09-2016  End *****
    ...
    public function uniqueExpenseReference(){
        $this->load->model('pos_model');
        $slip_no = $this->input->post('slip_no');
        //echo "hello world".$slip_no;die;
        $available = $this->pos_model->uniqueExpenseReference($slip_no);
        echo $available;
    }
     ...
     * code for add till
     */
/*  ************** Till Cmments By Anil 22-09-2016 For This Controllers *************** 
...
//            $status = $this->settings_model->addTill($this->input->post());
//            if($status == TRUE){
...
//		$this->load->model('settings_model');
//		//$this->sma->checkPermissions();
//		$this->data['executives'] = $this->site->getSalesExecutivesByBiller($_SESSION['biller_id'],5);
//		
//        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
//        $this->data['action'] = $action;
//		$this->data['tills'] = $this->settings_model->getAllTills();
...
//		$is_user_assigned_till = $this->settings_model->isUserAssignedToTill($data);
//		//echo json_encode($is_user_assigned_till);
//	
//		if(!$is_user_assigned_till){
//			$status = $this->settings_model->updateSingleTill($data);
//			if($status > 0){
...
//				$i++;
//			}	
//			$updated_rows = $this->settings_model->updateTills($updateData);
//			echo "<pre>";print_r($updated_rows[1]);
//			if($updated_rows[0] > 0){

...
class Products extends MY_Controller
{
    ...
    function index($warehouse_id = NULL)
    {
        //***** Added By Anil 20-09-2016 start****       
            ...
            if($arr_list[0]['products-list'] == NULL && (! $this->Owner)){
    ...
    function getProducts($warehouse_id = NULL)
    {
      ...
      /* Permission for action added By Anil 14-09-2016 Start */  
        ...
        $action .= '<li><a href="' . site_url() . 'assets/uploads/$2" data-type="image" data-toggle="lightbox"><i class="fa fa-file-photo-o"></i> '
            ...
            if($arr_prmsn[0]['products-delete'] != 1) {
                $delete_link = '';
            }
            if($arr_prmsn[0]['products-print_barcodes'] != 1) {
                 $single_barcode = '';
            }
            if($arr_prmsn[0]['products-print_labels'] != 1) {
                 $single_label = '';
            }
            if($arr_prmsn[0]['products-details'] != 1) {
                 $detail_link = '';
            }
            if($arr_prmsn[0]['products-duplicate'] == 1) {
                 $duplicate_link = '<a href="' . site_url('products/add/$1') . '"><i class="fa fa-plus-square"></i> ' . lang('duplicate_product') . '</a>';
            } else {
                 $duplicate_link = '';
            }
            if($arr_prmsn[0]['products-image'] == 1) {
                 $image_link = '<a href="' . site_url() . 'assets/uploads/$2" data-type="image" data-toggle="lightbox"><i class="fa fa-file-photo-o"></i> '
                    . lang('view_image') . '</a>';
            } else {
                 $image_link = '';
            }
            if($arr_prmsn[0]['products-edit'] == 1) {
                 $edit_link = '<a href="' . site_url('products/edit/$1') . '"><i class="fa fa-edit"></i> ' . lang('edit_product') . '</a>';
            } else {
                 $edit_link = '';
            }
            if($arr_prmsn[0]['products-adjust_quantity'] == 1) {
                 $adjust_quantity_link = '<a href="' . site_url('products/add_adjustment/$1/' . ($warehouse_id ? $warehouse_id : '')) . '" data-toggle="modal" data-target="#myModal"><i class="fa fa-filter"></i> '
                    . lang('adjust_quantity') . '</a>';
            } else {
                 $adjust_quantity_link = '';
           }
        ...
        $action .= '<li>' . $image_link . '</li>
			<li>' . $single_barcode . '</li>'
//			<li>' . $single_label . '</li>
			.'<li>' . $adjust_quantity_link . '</li>
				<li class="divider"></li>
                                <li>' . $delete_link . '</li>
			</ul>
		</div></div>';
        }
        /* Permission for action added By Anil 14-09-2016 End */  
        ...
        if ($warehouse_id) {
            ...
            ->select($this->db->dbprefix('products') . ".id as productid, "
                    ...
                    //. "wp.rack as rack, "
                    . $this->db->dbprefix('products') . ".lot_no as lot", FALSE)
            ->from('products');
            if ($this->Settings->display_all_products) {
                $this->datatables->join("( SELECT * from {$this->db->dbprefix('warehouses_products')} "
                . "WHERE warehouse_id = {$warehouse_id}) wp", 'products.id=wp.product_id', 'left');
//                if($category != 'all'){
...
//                }
            ...
            $this->datatables
                ->select($this->db->dbprefix('products') . ".id as productid, "
                        ...
                        . $this->db->dbprefix('products') . ".name as name, " 
                        . $this->db->dbprefix('categories') . ".name as cname, "
                        . "cost as cost, price as price, COALESCE(quantity, 0) as quantity,"
                       // . " unit, "
                        //. "NULL as rack, " 
                        . $this->db->dbprefix('products') . ".lot_no as lot", FALSE)
                ->from('products')
    ...
    function single_barcode($product_id = NULL, $warehouse_id = NULL)
    {
        ...
        if (!empty($options)) {
            ...
            for ($r = 1; $r <= $num; $r++) {
                ...
                $table .= '<td style="width: 20px;"><table class="table-barcode"><tbody><tr><td colspan="2" class="bold">' . $this->Settings->site_name . '</td></tr><tr><td colspan="2">' . $product->name . '</td></tr><tr><td colspan="2" class="text-center bc">' . $this->product_barcode($product->barcode, $product->barcode_symbology, 60) . '</td></tr>';
                foreach ($currencies as $currency) {
                    $table .= '<tr><td class="text-left">' . $currency->code . '</td><td class="text-right">' . $this->sma->formatMoney($product->price * $currency->rate) . '</td></tr>';
                }
                $table .= '</tbody></table>';
        ...
        if (!empty($options)) {
            ...
            foreach ($options as $option) {
                ...
                for($i=0; $i < $quantity; $i++) {
                    ...
                    $table .= '<td style="width: 20px;"><table class="table-barcode"><tbody><tr><td colspan="2" class="bold">' . $this->Settings->site_name . '</td></tr><tr><td colspan="2">' . $product->name . '</td></tr><tr><td colspan="2" class="text-center bc">' . $this->product_barcode($product->barcode, $product->barcode_symbology, 30) . '<br><strong>' . $option->name . '</strong><br>' . $this->product_barcode($product->barcode . '%' . $option->id, 'code39', 30) . '</td></tr>';
                    foreach ($currencies as $currency) {
                        $table .= '<tr><td class="text-left">' . $currency->code . '</td><td class="text-right">' . $this->sma->formatMoney($product->price * $currency->rate) . '</td></tr>';
                    }
                    $table .= '</tbody></table>';
            ...
            for ($r = 1; $r <= $num; $r++) {
                $table .= '<td style="width: 20px;"><table class="table-barcode"><tbody><tr><td colspan="2" class="bold">' . $this->Settings->site_name . '</td></tr><tr><td colspan="2">' . $product->name . '</td></tr><tr><td colspan="2" class="text-center bc">' . $this->product_barcode($product->barcode, $product->barcode_symbology, 30) . '</td></tr>';
                foreach ($currencies as $currency) {
                    $table .= '<tr><td class="text-left">' . $currency->code . '</td><td class="text-right">' . $this->sma->formatMoney($product->price * $currency->rate) . '</td></tr>';
                }
                $table .= '</tbody></table>';
    ...
    function single_label2($product_id = NULL, $warehouse_id = NULL)
    {
        ...
        if (!empty($options)) {
            $r = 1;
            foreach ($options as $option) {
                $html .= '<div class="labels"><strong>' . $pr->name . '</strong><br>' . $this->product_barcode($pr->barcode, $pr->barcode_symbology, 25) . '<br><span class="price">'.lang('price') .': ' .$this->Settings->default_currency. ' ' . $this->sma->formatMoney($pr->price) . '</span></div>';
                $r++;
            }
        } else {
            for ($r = 1; $r <= 16; $r++) {
    ...
    {
        //***** Added By Anil 20-09-2016 start****        
            $arr_print = $this->site->checkPermissions();  
            if($arr_print[0]['print_barcodes'] == NULL && (! $this->Owner)){
                $this->session->set_flashdata('error', lang("access_denied"));
                redirect('welcome'); 
            }
        //***** Added By Anil 20-09-2016 End****
        ...
        foreach ($products as $pr) {
            ...
            $html .= '<td colspan="2" class="text-center"><h3>' . $this->Settings->site_name . '</h3>' . $pr->name . '<br>' . $this->product_barcode($pr->barcode, $pr->barcode_symbology, 60);
    ...
    {
        //***** Added By Anil 20-09-2016 start****        
            $arr_print_labels = $this->site->checkPermissions();  
            if($arr_print_labels[0]['print_labels'] == NULL && (! $this->Owner)){
                $this->session->set_flashdata('error', lang("access_denied"));
                redirect('welcome'); 
            }
        //***** Added By Anil 20-09-2016 End****
        ...
        foreach ($products as $pr) {
            ...
            $html .= '<td class="text-center"><h4>' . $this->Settings->site_name . '</h4>' . $pr->name . '<br>' . $this->product_barcode($pr->barcode, $pr->barcode_symbology, 30);
            $html .= '<table class="table table-bordered">';
            foreach ($currencies as $currency) {
                $html .= '<tr><td class="text-left">' . $currency->code . '</td><td class="text-right">' . $this->sma->formatMoney($pr->price * $currency->rate) . '</td></tr>';
            }
	...
	//***** Added By Anil 20-09-2016 start****        
            ...
            if($arr_add[0]['products-add'] == NULL && (! $this->Owner)){
                $this->session->set_flashdata('error', lang("access_denied"));
                redirect('welcome'); 
            }
        //***** Added By Anil 20-09-2016 End****	
		...
		if(!empty($this->input->post('serialized'))){
			if($this->input->post('serialized') == 'on'){
				$serialized = true;
				$this->form_validation->set_rules('serial_number', lang("serial_number"), 'required|numeric|max_length[16]');
			}else{
				$serialized = false;
			}
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array(
				...
				'serialized' => !empty($serialized) ? $serialized : false,
				'serial_number' => !empty($serial_number) ? $serial_number : 0,			
            );
    ...
    {
        //***** Added By Anil 20-09-2016 start****        
            $arr_product_edit = $this->site->checkPermissions();  
            if($arr_product_edit[0]['products-edit'] == NULL && (! $this->Owner)){
                $this->session->set_flashdata('error', lang("access_denied"));
                redirect('welcome'); 
            }
        //***** Added By Anil 20-09-2016 End**** 
    ...
    {
        //***** Added By Anil 20-09-2016 start****        
            $arr_import = $this->site->checkPermissions();  
            if($arr_import[0]['import_product'] == NULL && (! $this->Owner)){
                $this->session->set_flashdata('error', lang("access_denied"));
                redirect('welcome'); 
            }
        //***** Added By Anil 20-09-2016 End****
    ...
    {
        //***** Added By Anil 20-09-2016 start****        
            $arr_upprice = $this->site->checkPermissions();  
            if($arr_upprice[0]['update_price'] == NULL &&(! $this->Owner)){
                $this->session->set_flashdata('error', lang("access_denied"));
                redirect('welcome'); 
            }
        //***** Added By Anil 20-09-2016 End**** 
    ...
    {
         //***** Added By Anil 20-09-2016 start****         
            $arr_quantity = $this->site->checkPermissions();  
            if($arr_quantity[0]['products-quantity_adjustments'] == NULL && (! $this->Owner)){
                $this->session->set_flashdata('error', lang("access_denied"));
                redirect('welcome'); 
            }
        //***** Added By Anil 20-09-2016 End**** 
    ...
    function getadjustments($pdf = NULL, $xls = NULL)
    {
        ...
        if ($pdf || $xls) {
            ...
            // Action permissions Modified By Anil Start
            if($this->Owner){                
                ...
                $arr = $this->site->checkPermissions();  
                if($arr[0]['products-quantity_delete']){
                    $delete_link = "<a href='#' class='tip po' title='<b>" . $this->lang->line("delete_adjustment") . "</b>' data-content=\"<p>"
                    . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' id='a__$1' href='" . site_url('products/delete_adjustment/$2') . "'>"
                    . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i></a>";
                }
            }
            // Action permissions Modified By Anil End
            $this->load->library('datatables');
            $this->datatables
                ->select($this->db->dbprefix('adjustments') . ".id as did, " . $this->db->dbprefix('adjustments') . ".product_id as productid, " . $this->db->dbprefix('adjustments') . ".date as date, " . $this->db->dbprefix('products') . ".image as image, " . $this->db->dbprefix('products') . ".code as code, " . $this->db->dbprefix('products') . ".name as pname, " . $this->db->dbprefix('product_variants') . ".name as vname, " . $this->db->dbprefix('adjustments') . ".quantity as quantity, ".$this->db->dbprefix('adjustments') . ".type, " . $this->db->dbprefix('warehouses') . ".name as wh");
    ...
    {
        //***** Added By Anil 20-09-2016 start****        
            $arr_product = $this->site->checkPermissions(); 
            if($arr_product[0]['products-edit'] == NULL && (! $this->Owner)){
                $this->session->set_flashdata('error', lang("access_denied"));
                redirect('welcome'); 
            }
        //***** Added By Anil 20-09-2016 End**** 
    ...
    { //echo $wh;echo "dddddddddddddddddddddddddddddddd";die;
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
                ...
                if ($this->input->post('form_action') == 'export_excel' || $this->input->post('form_action') == 'export_pdf') {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('E1', $biller_details->name);
                    if($this->Owner || $this->Admin){
                        ...
                        //$this->excel->getActiveSheet()->SetCellValue('D1', lang('subcategory_code'));
                        $this->excel->getActiveSheet()->SetCellValue('G2', lang('cost'));              
                        $this->excel->getActiveSheet()->SetCellValue('H2', lang('mrp'));
                        ...
                        //$this->excel->getActiveSheet()->SetCellValue('G1', lang('tax_rate'));
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('F2', lang('category'));
                        //$this->excel->getActiveSheet()->SetCellValue('D1', lang('subcategory_code'));            
                        $this->excel->getActiveSheet()->SetCellValue('G2', lang('mrp'));
...
//                    $this->excel->getActiveSheet()->SetCellValue('R1', lang('pcf6'));
                   ...
                   $this->excel->getActiveSheet()
                    ...
                    foreach ($_POST['val'] as $id) {
                        ...
                        if($this->Owner || $this->Admin){
                            $this->excel->getActiveSheet()->SetCellValue('D' . $row, $product->code);
                            $this->excel->getActiveSheet()->SetCellValue('E' . $row, $product->name);
                            $this->excel->getActiveSheet()->SetCellValue('F' . $row, $product->category_name);
                            //$this->excel->getActiveSheet()->SetCellValue('D' . $row, $product->subcategory_name);
                            ...
                            //$this->excel->getActiveSheet()->SetCellValue('G' . $row, $product->tax_rate_code);
                            //$this->excel->getActiveSheet()->SetCellValue('J' . $row, $product->tax_method ? lang('exclusive') : lang('inclusive'));
                            //$this->excel->getActiveSheet()->SetCellValue('K' . $row, $product->unit );
                            ...
                            //$this->excel->getActiveSheet()->SetCellValue('F' . $row, $product->tax_rate_code);
                            //$this->excel->getActiveSheet()->SetCellValue('I' . $row, $product->tax_method ? lang('exclusive') : lang('inclusive'));
                            //$this->excel->getActiveSheet()->SetCellValue('J' . $row, $product->unit );
...
//                        $this->excel->getActiveSheet()->SetCellValue('R' . $row, $product->cf6);
    ...
    function generate_barcode_png($product_code = NULL, $bcs = 'code39') {
        $product = $this->products_model->getProductByID($product_code);
        if(empty($product) || empty($product->code))
            return  false;
        $file_path = "../RC3.0.1.21/assets/uploads/barcode/";
        $product_code = $product->code;
        $product_barcode = $product->barcode;
        if(file_exists("$file_path$product_code.png"))
            return true;
        $this->load->library('zend');
        $this->zend->load('Zend/Barcode');
        $file = Zend_Barcode::draw('code128', 'image', array('text' => $product_barcode), array());
        if(!imagepng($file,"$file_path{$product_code}.png"))
            return false;
        return true;
    }

...
class Transfers_model extends CI_Model
{
    ...
    {
        $this->db->select('products.id, code, name, warehouses_products.quantity, cost, tax_rate, type, tax_method,inv_no,products.inv_date,products.hsn,products.lot_no')
            ->join('warehouses_products', 'warehouses_products.product_id=products.id', 'left')
            ->group_by('products.id');
        if ($this->Settings->overselling) {
            $this->db->where("type = 'standard' AND (barcode LIKE '%" . $term . "%' OR code LIKE '%" . $term . "%' OR  concat(inv_no, ' (', code, ')') LIKE '%" . $term . "%')");
        } else {
            $this->db->where("type = 'standard' AND warehouses_products.warehouse_id = '" . $warehouse_id . "' AND warehouses_products.quantity > 0 AND "
                . "(barcode LIKE '%" . $term . "%' OR code LIKE '%" . $term . "%' OR  concat(inv_no, ' (', code, ')') LIKE '%" . $term . "%')");
        }
        $this->db->limit($limit);
    ...
    public function updateQty($data = array(), $items = array())
    {
        if(!empty($data))
        {
            if($this->db->insert('purchase', $data))
            {
                ...
                foreach ($items as $item) {
                    $item['purchase_id'] = $last_insert;
                    if($this->db->insert('prchs_return_items', $item))
                    {
                        ...
                        ->set('quantity_balance', 'quantity_balance-'.$item['quantity'], false)

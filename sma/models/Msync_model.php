...
class Msync_model extends CI_Model {
    ...
    public function countryMaster_final() {
        $posDB = $this->posDbName();
        $intrDB = $this->intrDbName();
        $intrintrDB = $this->intrtointrDbName();
        $n = '$';
        $this->db->trans_begin();
        $this->db2->truncate($intrDB . ".intrm" . $n . "cntry");

        $q = $this->db3
                ->select('*')
                ->get($intrintrDB . ".intrm" . $n . "cntry");

        $result = $q->result();
        if ($result) {
            $aa = $this->db2->insert_batch($intrDB . ".intrm" . $n . "cntry", json_decode(json_encode($result), True));
            if ($aa) {
                if ($this->db->trans_status() === FALSE) {
                    $this->db->trans_rollback();
                } else {
                    $this->db->trans_commit();
                    $msg = "Country Records save successfully...";
                    return $msg;
                }
            } else {
    ...
    public function warehouseMaster() {
        ...
        if (!empty($arr)) {
            ...
            $w = "INSERT INTO " . $posDB . ".sma_warehouses(code,name,address,city,state,org_id,org_alias,tin_no,gst_no) SELECT a.ORG_ID,a.ORG_DESC,b.CNTRY_DESC,a.ORG_CITY,a.ORG_STATE,a.ORG_ID,a.ORG_ALIAS,a.ORG_TIN_NO,a.ORG_VAT_NO FROM " . $intrDB . ".intrm" . $n . "org a INNER JOIN " . $intrDB . ".intrm" . $n . "cntry b ON (a.ORG_COUNTRY_ID = b.CNTRY_ID) WHERE a.ORG_ID NOT IN(" . $arr . ")";
            if ($this->db->query($w)) {
                $w1 = "INSERT INTO " . $posDB . ".sma_companies(group_name,NAME,company,phone,country,address,city,state,postal_code,org_id,tin_no,vat_no,state_id) SELECT 'biller',a.ORG_DESC,a.ORG_DESC,'0123456789',b.CNTRY_DESC,a.ORG_ADD,a.ORG_CITY,a.ORG_STATE,a.ORG_PIN,a.ORG_ID,a.ORG_TIN_NO,a.ORG_VAT_NO,ORG_STATE_ID FROM " . $intrDB . ".intrm" . $n . "org a INNER JOIN " . $intrDB . ".intrm" . $n . "cntry b ON(a.ORG_COUNTRY_ID = b.CNTRY_ID) WHERE (a.ORG_ID NOT IN(" . $arr . ")) AND a.ORG_ID != '01'";
                if ($this->db->query($w1)) {
                    $msg = "warehouse Records save successfully...";
            ...
            $w = "INSERT INTO " . $posDB . ".sma_warehouses(code,name,address,city,state,org_id,org_alias,tin_no,gst_no) SELECT a.ORG_ID,a.ORG_DESC,b.CNTRY_DESC,a.ORG_CITY,a.ORG_STATE,a.ORG_ID,a.ORG_ALIAS,a.ORG_TIN_NO,a.ORG_VAT_NO FROM " . $intrDB . ".intrm" . $n . "org a INNER JOIN " . $intrDB . ".intrm" . $n . "cntry b ON (a.ORG_COUNTRY_ID = b.CNTRY_ID) WHERE a.POS_ORG_ID = '00'";
            if ($this->db->query($w)) {
                $w1 = "INSERT INTO " . $posDB . ".sma_companies(group_name,NAME,company,phone,country,address,city,state,postal_code,org_id,tin_no,vat_no,state_id) SELECT 'biller',a.ORG_DESC,a.ORG_DESC,'0123456789',b.CNTRY_DESC,a.ORG_ADD,a.ORG_CITY,a.ORG_STATE,a.ORG_PIN,a.ORG_ID,a.ORG_TIN_NO,a.ORG_VAT_NO,a.ORG_STATE_ID FROM " . $intrDB . ".intrm" . $n . "org a INNER JOIN " . $intrDB . ".intrm" . $n . "cntry b ON(a.ORG_COUNTRY_ID = b.CNTRY_ID) WHERE (a.POS_ORG_ID ='00') AND a.ORG_ID != '01'";
                if ($this->db->query($w1)) {
                    $msg = "warehouse Records save successfully...";
                    $w2 = "UPDATE " . $intrDB . ".intrm" . $n . "org a INNER JOIN " . $posDB . ".sma_warehouses b ON(a.ORG_ID = b.org_id) SET a.POS_ORG_ID = b.id WHERE a.ORG_ID = b.org_id";
                    $this->db->query($w2);
    ...
    public function userMaster_final($org) {
        ...
        ////uu
        //uu
        // AND USR_ID NOT IN(SELECT USR_ID FROM  ".$intrDB.".intrm".$n."usr) 
        ...
        $result = $q->result();
        if ($result) {
            $record1 = json_decode(json_encode($result), True);
            $count = 0;
            foreach ($record1 as $rows) {
                $insert_query = $this->db2->insert_string($intrDB . ".intrm" . $n . "usr", $rows);
    ...
    public function taxMaster_final($org) {
        ...
        //$this->db2->truncate($intrDB.".intrm".$n."org");
        //uu
        /* echo "SELECT GRP_ID FROM ".$intrDB.".intrm".$n."itm".$n."grp".$n."org WHERE ".$intrDB.".intrm".$n."itm".$n."grp".$n."org.GRP_ID = ".$intrintrDB.".intrm".$n."itm".$n."grp".$n."org.GRP_ID ";exit; */

        $where = "ORG_ID='" . $org . "' AND POS_GRP_ID ='00'";
        // AND GRP_ID NOT IN( SELECT GRP_ID FROM ".$intrDB.".intrm".$n."itm".$n."grp".$n."org)
        $this->db3
                ->select('*')
                ->where($where);
...
//         print_r($result1);exit;
        ...
        if ($result && $result2) {
            ...
            $record = json_decode(json_encode($result));
            foreach ($record as $row) {
                $insert_query = $this->db2->insert_string($intrDB . ".intrm" . $n . "itm" . $n . "grp" . $n . "org", $row);
                $insert_query = str_replace('INSERT INTO', 'INSERT IGNORE INTO', $insert_query);
                $this->db2->query($insert_query);
            ...
            $record2 = json_decode(json_encode($result2));
            foreach ($record2 as $row2) {
                $insert_query = $this->db2->insert_string($intrDB . ".intrm" . $n . "itm" . $n . "grp", $row2);
                $insert_query = str_replace('INSERT INTO', 'INSERT IGNORE INTO', $insert_query);
                $this->db2->query($insert_query);
                // $aa =  $this->db2->insert_batch($intrDB.".intrm".$n."itm".$n."grp".$n."org",json_decode(json_encode($result), True));
            }
    ...
    public function categoryMaster($org) {
        ...
        $w = "INSERT IGNORE INTO " . $posDB . ".sma_categories(code,name,tax_rate_id) SELECT a.GRP_ID,a.GRP_NM,'0' FROM " . $intrDB . ".intrm" . $n . "itm" . $n . "grp a WHERE (a.POS_GRP_ID = '00')";
        if ($this->db->query($w)) {
    ...
    public function customerMaster_final($org) {
        ...
        //$this->db2->truncate($intrDB.".intrm".$n."org");
        //uu
        $where = "ORG_ID='" . $org . "' AND EO_TYPE ='C' AND POS_EO_ID='00' AND EO_ID>0";
        // for eo & eo_org
        $this->db3
                ->select('*')
                ->where($where);
        ...
        if ($result && $result2 && $result5 && $result7) {
            ...
            if ($aa && $aa1 && $aa3 && $aa4) {
                ...
                if ($this->customerMaster($org)) {
                    ...
                    $where1 = "ORG_ID='" . $org . "' AND EO_TYPE ='C' AND CREATE_FLG ='1' AND POS_EO_ID !='00'";
...
//echo "hi..."; die;
    ...
    public function productMaster_final($org) {
        ...
        if ($result && $result2 && $result3) {
            ...
            $count = 0;
            foreach ($record1 as $rows) {
                $insert_query = $this->db2->insert_string($intrDB . ".intrm" . $n . "prod" . $n . "org", $rows);
                $insert_query = str_replace('INSERT INTO', 'INSERT IGNORE INTO', $insert_query);
            ...
            $record2 = json_decode(json_encode($result2), True);
            $count1 = 0;
            foreach ($record2 as $rows2) {
                $insert_query = $this->db2->insert_string($intrDB . ".intrm" . $n . "prod", $rows2);
                $insert_query = str_replace('INSERT INTO', 'INSERT IGNORE INTO', $insert_query);
                $this->db2->query($insert_query);
                $count1++;
                // $aa =  $this->db2->insert_batch($intrDB.".intrm".$n."itm".$n."grp".$n."org",json_decode(json_encode($result), True));
            ...
            $record3 = json_decode(json_encode($result3), True);
            $count2 = 0;
            foreach ($record3 as $rows3) {
                $insert_query = $this->db2->insert_string($intrDB . ".intrm" . $n . "itm" . $n . "stock" . $n . "dtl", $rows3);
                $insert_query = str_replace('INSERT INTO', 'INSERT IGNORE INTO', $insert_query);
...
// die;
    ...
    public function productMaster($org) {
        ...
        $w = "INSERT IGNORE INTO " . $posDB . ".sma_products(code,name,unit,cost,price,category_id,subcategory_id,quantity,tax_rate,details,warehouse,barcode,barcode_symbology,product_details,serialized,lot_no,sr_no,org_id,grp_id,hsn,promotion,tax_percentage) SELECT a.ITM_ID,b.ITM_DESC,b.UOM_BASIC,c.UOM_BASIC,c.PRICE_SLS,f.id,b.UOM_BASIC,c.AVAIL_QTY,g.id,b.ITM_LONG_DESC,e.id,b.ITM_LEGACY_CODE,'code128',b.ITM_LONG_DESC,b.SERIALIZED_FLG,c.LOT_NO,c.SR_NO,a.ORG_ID,b.GRP_ID,b.HSN,b.PROMOTION,b.TAX_PERCENTAGE FROM " . $intrDB . ".intrm" . $n . "prod" . $n . "org a INNER JOIN " . $intrDB . ".intrm" . $n . "prod b ON b.ITM_ID = a.ITM_ID INNER JOIN " . $posDB . ".sma_warehouses e ON e.org_id = a.ORG_ID INNER JOIN " . $posDB . ".sma_categories f ON f.code = b.GRP_ID LEFT JOIN   " . $intrDB . ".intrm" . $n . "itm" . $n . "stock" . $n . "dtl c ON (c.ITM_ID = a.ITM_ID AND c.ORG_ID = a.ORG_ID) LEFT JOIN " . $posDB . ".sma_tax_rates g ON (g.org_id = a.ORG_ID AND g.grp_id = b.GRP_ID) WHERE a.POS_ITM_ID = '00'";
    ...
    public function purchaseItemMaster($org) {
        ...
        $n = '$';
        //$this->db->trans_begin();
        $w = "INSERT INTO " . $posDB . ".sma_purchase_items(product_id,product_code,product_name,net_unit_cost,quantity,warehouse_id,item_tax,tax_rate_id,tax,subtotal,quantity_balance,date,status,unit_cost,lot_no,sr_no,org_id,grp_id) SELECT a.id,a.code,a.name,(a.price -((a.price * a.tax_percentage) /(100 + a.tax_percentage))) AS unit_cost,a.quantity,a.warehouse,((a.price * a.tax_percentage) /(100 + a.tax_percentage)) AS item_tax,a.tax_rate,a.tax_percentage,a.price,a.quantity,curdate(),'received',a.price,a.lot_no,a.sr_no,a.org_id,a.grp_id FROM " . $posDB . ".sma_products a WHERE a.trsfr_flg IS NULL OR a.trsfr_flg = ''";

        // INNER JOIN ".$posDB.".sma_tax_rates b ON(b.id = a.tax_rate AND b.org_id = a.org_id AND b.grp_id = a.grp_id) 
        if ($this->db->query($w)) {
    ...
    public function purchaseItemUPD($org) {
        ...
        $w = "UPDATE " . $posDB . ".sma_purchase_items z INNER JOIN " . $posDB . ".sma_products a ON a.id= z.product_id SET z.product_code = a.code, z.product_name = a.name,z.net_unit_cost = (a.price -((a.price * a.tax_percentage) /(100 + a.tax_percentage))), z.quantity = a.quantity, z.item_tax =  ((a.price * a.tax_percentage) /(100 + a.tax_percentage)), z.tax_rate_id = a.tax_rate, z.tax = a.tax_percentage, z.subtotal = a.price, z.quantity_balance = a.quantity, z.lot_no = a.lot_no, z.sr_no = a.sr_no, z.org_id = a.org_id, z.grp_id = a.grp_id WHERE a.tupd_flg= '1'";
    ...
    public function eoMaster($org) {
        ...
        $n = '$';
        $this->db->trans_begin();
        $w = "INSERT INTO " . $intrDB . ".intrm" . $n . "eo(CLD_ID,SLOC_ID,HO_ORG_ID,EO_TYPE,POS_EO_ID,ORG_ID,EO_CATG_ID,EO_NM,EO_PHONE,VAT_NO,USR_ID_CREATE,USR_ID_CREATE_DT,USR_ID_MOD,USR_ID_MOD_DT,SLTN,SYNC_FLG) SELECT '0000','1','N','C',a.id,a.org_id,'T002',CONCAT(a.name,' ',a.lname),a.phone,a.vat_no,'1',curdate(),'1',curdate(),a.salutation,'1' FROM " . $posDB . ".sma_companies a WHERE a.id!= '1' AND a.group_name!='biller' AND (a.sync_flg IS NULL OR a.sync_flg= '')";
        if ($this->db->query($w)) {
            $w1 = "INSERT INTO " . $intrDB . ".intrm" . $n . "eo" . $n . "add(CLD_ID,SLOC_ID,HO_ORG_ID,EO_TYPE,POS_EO_ID,ORG_ID,USR_ID_CREATE,USR_ID_CREATE_DT,USR_ID_MOD,USR_ID_MOD_DT,ADDRESS,SYNC_FLG) SELECT '0000','1','N','C',a.id,a.org_id,'1',curdate(),'1',curdate(),a.address,'1' FROM " . $posDB . ".sma_companies a WHERE a.id!= '1' AND a.group_name!='biller' AND (a.sync_flg IS NULL OR a.sync_flg= '')";
  ...
  'NULL',a.discount,a.subtotal,'1' FROM " . $posDB . ".sma_sale_items a INNER JOIN " . $posDB . ".sma_warehouses b ON (a.warehouse_id = b.id) INNER JOIN " . $posDB . ".sma_products c ON (a.product_id = c.id) WHERE a.sync_flg IS NULL OR a.sync_flg = ''";
    ...
    public function purchaseReturnMaster($org) {
        $posDB = $this->posDbName();
        $intrDB = $this->intrDbName();
        $n = '$';
        $this->db->trans_begin();
        $w = "INSERT INTO " . $intrDB . ".intrm" . $n . "pur" . $n . "return(CLD_ID,SLOC_ID,HO_ORG_ID,ORG_ID, POS_PR_ID,DATE,REF_NO,USR_ID,SYNC_FLG) SELECT '0000','1','N',b.org_id,a.id,DATE_FORMAT(a.date,'%d-%m-%Y'),a.reference_no,c.usr_id,'1' FROM " . $posDB . ".sma_purchase a INNER JOIN " . $posDB . ".sma_warehouses b ON (a.warehouse_id = b.id) INNER JOIN " . $posDB . ".sma_users c ON (c.id = a.created_by) WHERE a.sync_flg IS NULL OR a.sync_flg = ''";
        if ($this->db->query($w)) {
            $count = $this->db->affected_rows();
            if ($count > 0) {
                $w1 = "UPDATE " . $posDB . ".sma_purchase a INNER JOIN " . $intrDB . ".intrm" . $n . "pur" . $n . "return b ON a.id = b.POS_PR_ID SET a.sync_flg='1' WHERE a.id = b.POS_PR_ID AND (a.sync_flg IS NULL OR a.sync_flg = '') ";
                $q = $this->db->query($w1);
            }
            // added by vikas singh for transactions
            ...
            if ($this->db->trans_status() === FALSE) {
                ...
                $this->db->trans_commit();
                $msg = "Purchase Return Sync Successfully....";
                return $msg;
            }
    ...
    public function purchaseReturnMaster_final($org) {
        if ($this->purchaseReturnMaster($org)) {
            $posDB = $this->posDbName();
            $intrDB = $this->intrDbName();
            $intrintrDB = $this->intrtointrDbName();
            $n = '$';
            ...
            $this->db2
                    ->select('*')
                    ->where('SYNC_FLG', '1');
            $q = $this->db2->get($intrDB . ".intrm" . $n . "pur" . $n . "return");

            $result = $q->result();
            if ($result) {

                $aa = $this->db3->insert_batch($intrintrDB . ".intrm" . $n . "pur" . $n . "return", json_decode(json_encode($result), True));
                ...
                if ($aa) {
                    ...
                    $this->db2->where('SYNC_FLG', '1');
                    $finalres1 = $this->db2->update($intrDB . ".intrm" . $n . "pur" . $n . "return", $crflag);
                    if ($finalres1) {
...
//// ----------------- Purchase Return Item ------------------------
    ...
    public function purchaseReturnItemMaster($org) {
        $posDB = $this->posDbName();
        $intrDB = $this->intrDbName();
        $n = '$';
        $this->db->trans_begin();
        $w = "INSERT INTO " . $intrDB . ".intrm" . $n . "pur" . $n . "return" . $n . "item(CLD_ID,SLOC_ID,HO_ORG_ID,ORG_ID,POS_PR_ITM_ID,POS_PR_ID,INV_NO,INV_DATE,ITM_ID, ITM_DESC,ITM_HSN,QTY,LOT_NO,SYNC_FLG) SELECT '0000','1','N',b.org_id,a.id,a.purchase_id,a.invoice_no,DATE_FORMAT(a.invoice_date,'%d-%m-%Y'),a.product_code,a.product_name,a.product_hsn,a.quantity,a.lot_no,'1'  FROM " . $posDB . ".sma_prchs_return_items a INNER JOIN " . $posDB . ".sma_warehouses b ON (a.warehouse_id = b.id) WHERE a.sync_flg IS NULL OR a.sync_flg = ''";
        if ($this->db->query($w)) {
            $count = $this->db->affected_rows();
            if ($count > 0) {
                $w1 = "UPDATE " . $posDB . ".sma_prchs_return_items a INNER JOIN " . $intrDB . ".intrm" . $n . "pur" . $n . "return" . $n . "item b ON a.id = b.POS_PR_ITM_ID SET a.sync_flg='1' WHERE a.id = b.POS_PR_ITM_ID AND (a.sync_flg IS NULL OR a.sync_flg = '') ";
                $q = $this->db->query($w1);
            }
            // added by vikas singh for transactions
            ...
            if ($this->db->trans_status() === FALSE) {
                ...
                $this->db->trans_commit();
                $msg = "Purchase Return Item Sync Successfully....";
                return $msg;
            }
    ...
    public function purchaseReturnItemMaster_final($org) {
        if ($this->purchaseReturnItemMaster($org)) {
            $posDB = $this->posDbName();
            $intrDB = $this->intrDbName();
            $intrintrDB = $this->intrtointrDbName();
            $n = '$';
            ...
            $this->db2
                    ->select('*')
                    ->where('SYNC_FLG', '1');
            $q = $this->db2->get($intrDB . ".intrm" . $n . "pur" . $n . "return" . $n . "item");

            $result = $q->result();
            if ($result) {

                $aa = $this->db3->insert_batch($intrintrDB . ".intrm" . $n . "pur" . $n . "return" . $n . "item", json_decode(json_encode($result), True));
                ...
                if ($aa) {
                    ...
                    $this->db2->where('SYNC_FLG', '1');
                    $finalres1 = $this->db2->update($intrDB . ".intrm" . $n . "pur" . $n . "return" . $n . "item", $crflag);
                    if ($finalres1) {
                        $msg = "Purchase Return Item Successfully....";
                        return $msg;
                    } else {
...
//// -----------------End Purchase Return Item ---------------------    
    ...
    // I.T.-09 (Pos -> intermediate) Expense Details Sync


    public function ExpenseMaster($org) {
        $posDB = $this->posDbName();
        $intrDB = $this->intrDbName();
        $n = '$';
        $this->db->trans_begin();
        $w = "INSERT INTO " . $intrDB . ".intrm" . $n . "expense(CLD_ID,SLOC_ID,HO_ORG_ID,ORG_ID,POS_EXP_ID,DATE,REFERENCE,AMOUNT,BANK,NOTE,USR_ID,ATMNT,REG_ID) SELECT '0000','1','N',c.org_id,a.id,DATE_FORMAT(a.date,'%Y-%m-%d'),a.reference,a.amount,a.bank,a.note,b.usr_id,a.attachment,'NULL' FROM " . $posDB . ".sma_expenses a INNER JOIN " . $posDB . ".sma_users b ON (a.created_by = b.id) INNER JOIN " . $posDB . ".sma_warehouses c ON (b.warehouse_id = c.id) WHERE
  a.sync_flg IS NULL OR a.sync_flg= ''";
        //echo $w; die;
        if ($this->db->query($w)) {
            $count = $this->db->affected_rows();
            if ($count > 0) {
                $w1 = "UPDATE " . $posDB . ".sma_expenses a INNER JOIN " . $intrDB . ".intrm" . $n . "expense b ON (a.id = b.POS_EXP_ID) SET a.sync_flg = '1',b.SYNC_FLG = '1' WHERE a.id = b.POS_EXP_ID";
                $q = $this->db->query($w1);
            }
            // added by vikas singh for transactions
    ...
    public function ExpenseMaster_final($org) {
        if ($this->ExpenseMaster($org)) {
            $posDB = $this->posDbName();
            $intrDB = $this->intrDbName();
            $intrintrDB = $this->intrtointrDbName();
            $n = '$';
            ...
            $this->db2
                    ->select('*')
                    ->where('SYNC_FLG', '1');
            $q = $this->db2->get($intrDB . ".intrm" . $n . "expense");

            $result = $q->result();

            if ($result) {

                $aa = $this->db3->insert_batch($intrintrDB . ".intrm" . $n . "expense", json_decode(json_encode($result), True));
                ...
                if ($aa) {
                    ...
                    $this->db2->where('SYNC_FLG', '1');
                    $finalres1 = $this->db2->update($intrDB . ".intrm" . $n . "expense", $crflag);
                    if ($finalres1) {
                        $msg = "BANK Details Sync Successfully....";
                        return $msg;
                    } else {
...
// end SMA_EXPENSES
    ...
    '1',curdate(),a.cc_nation,a.cc_edc  FROM " . $posDB . ".sma_payments a INNER JOIN " . $posDB . ".sma_sales b ON (a.sale_id = b.id) INNER JOIN " . $posDB . ".sma_warehouses c ON (b.warehouse_id = c.id) INNER JOIN " . $posDB . ".sma_companies d ON (b.customer_id = d.id) WHERE
  a.sync_flg IS NULL OR a.sync_flg= ''";
        //echo $w; die;
        if ($this->db->query($w)) {
            $count = $this->db->affected_rows();
    ...
    public function cvMaster($invoice_no) {
        ...
        if ($this->db->query($w)) {
            $w1 = "UPDATE " . $intrDB . ".intrm" . $n . "credit" . $n . "voucher" . $n . "dtl a INNER JOIN " . $posDB . ".sma_gift_cards b ON (b.invoice_no = a.INVOICE_NO AND b.card_no = a.CARD_NO) SET a.COUNT_FLG='2' WHERE (b.invoice_no = a.INVOICE_NO AND b.card_no = a.CARD_NO)";
    ...
    public function Change_sync_status() {
        $posDB = $this->posDbName();
        $intrDB = $this->intrDbName();
        $intrintrDB = $this->intrtointrDbName();
        $n = '$';
        $w = "SELECT syncing_status FROM " . $posDB . ".sma_pos_settings ";
        $q = $this->db->query($w);
        $r = $q->result();
        $s = $r[0]->syncing_status;
        if ($s == 'sync_out') {
            $w1 = "UPDATE " . $posDB . ".sma_pos_settings SET syncing_status = 'sync_in' WHERE syncing_status = 'sync_out' ";
            $q1 = $this->db->query($w1);
            return true;
        }
        if ($s == 'sync_in') {
            $w2 = "UPDATE " . $posDB . ".sma_pos_settings SET syncing_status = 'sync_out' WHERE syncing_status = 'sync_in' ";
            $q2 = $this->db->query($w2);
            return true;
        }
    }

    public function Credit_sync_status() {
        $posDB = $this->posDbName();
        $intrDB = $this->intrDbName();
        $intrintrDB = $this->intrtointrDbName();
        $n = '$';
        $w = "SELECT cr_syncing_status FROM " . $posDB . ".sma_pos_settings ";
        $q = $this->db->query($w);
        $r = $q->result();
        $s = $r[0]->cr_syncing_status;
        if ($s == 'sync_out') {
            $w1 = "UPDATE " . $posDB . ".sma_pos_settings SET cr_syncing_status = 'sync_in' WHERE cr_syncing_status = 'sync_out' ";
            $q1 = $this->db->query($w1);
            return true;
        }
        if ($s == 'sync_in') {
            $w2 = "UPDATE " . $posDB . ".sma_pos_settings SET cr_syncing_status = 'sync_out' WHERE cr_syncing_status = 'sync_in' ";
            $q2 = $this->db->query($w2);
            return true;
        }
    }

    public function Reverse_sync_status($org) {
        $posDB = $this->posDbName();
        $intrDB = $this->intrDbName();
        $intrintrDB = $this->intrtointrDbName();
        $n = '$';
        $w = "SELECT * FROM " . $intrintrDB . ".intrm" . $n . "reverse" . $n . "sync" . $n . "status ORDER BY ID DESC limit 1 ";
        $q = $this->db3->query($w);
        $r = $q->result();
        //print_r($q->result()); die;
        if (empty($r)) {
            $w1 = "INSERT INTO " . $intrintrDB . ".intrm" . $n . "reverse" . $n . "sync" . $n . "status(ORG_ID,DATE,FLG)  VALUES ('" . $org . "', NOW(), '0') ";
            $q1 = $this->db3->query($w1);
            return 1;
        } else {
            $s = $r[0]->FLG;
            if ($s == '1') {
                $w2 = "INSERT INTO " . $intrintrDB . ".intrm" . $n . "reverse" . $n . "sync" . $n . "status(ORG_ID,DATE,FLG)  VALUES ('" . $org . "', NOW(), '0') ";
                $q2 = $this->db3->query($w2);
                return 1;
            } else {
                return 0;
            }
    ...
    public function Change_reverse_sync_flg() {
        $posDB = $this->posDbName();
        $intrDB = $this->intrDbName();
        $intrintrDB = $this->intrtointrDbName();
        $n = '$';
        $w = "UPDATE " . $intrintrDB . ".intrm" . $n . "reverse" . $n . "sync" . $n . "status SET FLG = '1' WHERE FLG = '0' ";
        $q = $this->db3->query($w);
        return true;
    }
    ...
    public function syncToIntermediate($org) {
        if ($this->Reverse_sync_status($org) == '1') {
            $this->eoMaster_final($org);
            $this->slsInvMaster_final($org); // invoice master
            $this->slsInvItmMaster_final($org); // invoice item
            $this->slsRmaMaster_final($org); //sales return masters 
            $this->slsRmaItmMaster_final($org); //sales return masters item
            $this->PaymntMaster_final($org); // payment details sync
            $this->creditVoucherMaster_final($org); //credit voucher details
            $this->ExpenseMaster_final($org); //bank details saved
            $this->emrsInvMaster_final($org); //Interstate invoice master
            $this->emrsInvItmMaster_final($org); //interstate invoice master item
            $this->purchaseReturnMaster_final($org); // For Purchase return
            $this->purchaseReturnItemMaster_final($org); // For Purchase return item
            $this->Change_sync_status(); //
            $this->Credit_sync_status(); // status of offline payment 
            $this->Change_reverse_sync_flg();
            return true;
        } else {
            return false;
        }
    ...
    // Empty POS DB
    public function emptyPOS() {  //echo "hi..."; die;
    ...
    public function getwarehouselist() {
        ...
        $this->db3->select('ORG_ID,ORG_DESC');
        if (isset($_SESSION['org_id'])) {
            $where = $this->db3->where('org_id', $_SESSION['org_id']);
        }
        $q = $this->db3->get($intrintrDB . ".intrm" . $n . "org");
        $result = $q->result();
        //print_r($result); die;
        return $result;
    }

    public function emrsInvMaster_final($org) {

        if ($this->emrsInvMaster($org)) {
            $posDB = $this->posDbName();
            $intrDB = $this->intrDbName();
            $intrintrDB = $this->intrtointrDbName();
            $n = '$';
            ...
            $this->db2
                    ->select('*')
                    ->where('SYNC_FLG', '1');
            $q = $this->db2->get($intrDB . ".intrm" . $n . "emrs" . $n . "inv");

            $result = $q->result();

            if ($result) {

                $aa = $this->db3->insert_batch($intrintrDB . ".intrm" . $n . "emrs" . $n . "inv", json_decode(json_encode($result), True));
                ...
                if ($aa) {
                    ...
                    $this->db2->where('SYNC_FLG', '1');
                    $finalres1 = $this->db2->update($intrDB . ".intrm" . $n . "emrs" . $n . "inv", $crflag);
                    if ($finalres1) {
                        $msg = "Sales Details Sync Successfully....";
                        return $msg;
                    } else {
    ...
    public function emrsInvMaster($org) {
        $posDB = $this->posDbName();
        $intrDB = $this->intrDbName();
        $n = '$';
        $this->db->trans_begin();
        $w = "INSERT INTO " . $intrDB . ".intrm" . $n . "emrs" . $n . "inv(CLD_ID,SLOC_ID,HO_ORG_ID,ORG_ID,POS_DOC_ID,DOC_ID,DOC_DT,EO_ID,POS_EO_ID,SLS_REP_ID,POS_SLS_REP_ID,TAX_BASIS,TAX_ID,TAX_VAL,DISC_BASIS,DISC_TYPE,POS_INV_TOTAL,DISC_VAL,DISC_TOTAL_VAL,SYNC_FLG) SELECT '0000','1','N',b.org_id,a.id,a.reference_no,DATE_FORMAT(a.date,'%Y-%m-%d'),c.usr_id,a.customer_id,d.usr_id,a.sales_executive_id,'NULL',a.order_tax_id,a.total_tax,'NULL','NULL',a.grand_total,a.total_discount,a.total_discount,'1' FROM " . $posDB . ".sma_emrs a INNER JOIN " . $posDB . ".sma_warehouses b ON (a.warehouse_id = b.id) INNER JOIN " . $posDB . ".sma_users c ON (a.customer_id = c.id) INNER JOIN " . $posDB . ".sma_users d ON (a.sales_executive_id = d.id) WHERE a.sync_flg IS NULL OR a.sync_flg = ''";
        if ($this->db->query($w)) {
            $count = $this->db->affected_rows();
            if ($count > 0) {
                $w1 = "UPDATE " . $posDB . ".sma_emrs a INNER JOIN " . $intrDB . ".intrm" . $n . "emrs" . $n . "inv b ON a.id = b.POS_DOC_ID SET a.sync_flg = '1' WHERE a.id = b.POS_DOC_ID";
                $q = $this->db->query($w1);
            }
            $msg = "Sales Details Sync Successfully....";
    ...
    public function emrsInvItmMaster_final($org) {
        if ($this->emrsInvItmMaster($org)) {
            $posDB = $this->posDbName();
            $intrDB = $this->intrDbName();
            $intrintrDB = $this->intrtointrDbName();
            $n = '$';
            ...
            $this->db2
                    ->select('*')
                    ->where('SYNC_FLG', '1');
            $q = $this->db2->get($intrDB . ".intrm" . $n . "emrs" . $n . "inv" . $n . "itm");

            $result = $q->result();
            if ($result) {

                $aa = $this->db3->insert_batch($intrintrDB . ".intrm" . $n . "emrs" . $n . "inv" . $n . "itm", json_decode(json_encode($result), True));
                ...
                if ($aa) {
                    ...
                    $this->db2->where('SYNC_FLG', '1');
                    $finalres1 = $this->db2->update($intrDB . ".intrm" . $n . "emrs" . $n . "inv" . $n . "itm", $crflag);
                    if ($finalres1) {
                        $msg = "EMRS Item Details Sync Successfully....";
                        return $msg;
                    } else {
    ...
    public function emrsInvItmMaster($org) {
        $posDB = $this->posDbName();
        $intrDB = $this->intrDbName();
        $n = '$';
        $this->db->trans_begin();
        $w = "INSERT INTO " . $intrDB . ".intrm" . $n . "emrs" . $n . "inv" . $n . "itm(CLD_ID,SLOC_ID,HO_ORG_ID,ORG_ID,POS_DOC_ID,DOC_ID,ITM_ID,POS_ITM_ID,SR_NO,LOT_NO,ITM_QTY,   POS_ITM_UOM_BS,SLS_PRICE,TAX_VAL,DISC_TOTAL_VAL,DISC_TYPE,DISC_VAL,ITM_TOTAL_VAL,SYNC_FLG) SELECT '0000','1','N',b.org_id,a.id,a.sale_id,a.product_code,a.product_id,c.sr_no,c.lot_no,a.quantity,a.net_unit_price,a.real_unit_price,a.item_tax,a.item_discount,
                'NULL',a.discount,a.subtotal,'1' FROM " . $posDB . ".sma_emrs_items a INNER JOIN " . $posDB . ".sma_warehouses b ON (a.warehouse_id = b.id) INNER JOIN " . $posDB . ".sma_products c ON (a.product_id = c.id) WHERE a.sync_flg IS NULL OR a.sync_flg = ''";
        if ($this->db->query($w)) {
            $count = $this->db->affected_rows();
            if ($count > 0) {
                $w1 = "UPDATE " . $posDB . ".sma_emrs_items a INNER JOIN " . $intrDB . ".intrm" . $n . "emrs" . $n . "inv" . $n . "itm b ON a.id = b.POS_DOC_ID SET a.sync_flg='1' WHERE a.id = b.POS_DOC_ID";
                $q = $this->db->query($w1);
            }
            // added by vikas singh for transactions
    ...
    public function checkInvoiceEoId($invoice_no) {
        $posDB = $this->posDbName();
        $intrDB = $this->intrDbName();
        $intrintrDB = $this->intrtointrDbName();
        $n = '$';
        $where = "INVOICE_NO='" . $invoice_no . "' AND SYNC_FLG ='1' ";
        $this->db3->select('*')->where($where);
        $result = $this->db3->get($intrintrDB . ".intrm" . $n . "credit" . $n . "voucher" . $n . "dtl")->result();

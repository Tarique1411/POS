...
class Products_model extends CI_Model
{
    ...
    {   
        //$q = $this->db->query('SELECT sma_products.*, cost, price, tax_rate, SUM(quantity) as bal_qty FROM `sma_products`  WHERE quantity >0 GROUP BY code');
        $query = "SELECT
                sma_products.id,
              ...
              WHERE
                sma_warehouses_products.warehouse_id = " . $this->session->userdata('warehouse_id') . " AND sma_warehouses_products.quantity > 0 AND sma_products.cart_qty=1
              GROUP BY
                sma_products.code";
    ...
    {
        $this->db->select($this->db->dbprefix('products') . '.*, ' . $this->db->dbprefix('tax_rates') . '.code as tax_rate_code, ' . $this->db->dbprefix('categories') . '.name as category_name, ' . $this->db->dbprefix('subcategories') . '.name as subcategory_name')
    ...
    public function getProductNames($term, $limit = 5)
    {
        ...
        $this->db->join('product_variants', 'product_variants.product_id=products.id', 'left')
            ...
            ->group_by('products.name')->limit($limit);
        $q = $this->db->get('products');
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
	...
	public function getProductsByCategoryId($id)
        {
            ...
            //if($_SESSION['warehouse_id']==''){$_SESSION['warehouse_id']=2;}
//            $this->db->select("products.id,products.code, products.name, ".$this->db->dbprefix('purchase_items').".unit_cost," . $this->db->dbprefix('purchase_items') . ".quantity_balance as balance")
//                ->join('purchase_items', 'products.id = purchase_items.product_id','left')
//                ->where('purchase_items.warehouse_id',$_SESSION['warehouse_id']);
//            $q = $this->db->get_where('products', array('category_id' => $id,'products.quantity >='=>1));
            $sql = "select 
                    ...
                    " . $this->db->dbprefix('purchase_items') . ".quantity_balance as balance,
                    ...
                    " . $this->db->dbprefix('products') . ".quantity >= 1
                    group by
    ...
    public function getSalesExeList(){
        $salesList = $this->db->select("*")
        ->from("users")

...
class Auth_model extends CI_Model
{
	...
	//	die("vivekekekekekekkek");
        ...
        $query = $this->db->select($this->identity_column . ', username, email, id, password, active, last_login, last_ip_address, avatar, gender, group_id, warehouse_id, biller_id, company_id, show_cost, show_price, org_id')
        ...
        $user_id=$user->id;
        $query1= $this->db->select('till_ip')
          ->where('user_id', $user_id)
          ->get('till');
          
	$till=$query1->result();
       
        $till_ip=$till[0]->till_ip;
        $user_ip= $user->last_ip_address;
            ...
            if ($password === TRUE) {
              ...
              /*
               / For check the validation of user group & store @ Ankit
               */
              /* if($user->group_id!= $group_id)
               {
                   $this->set_error('please_select_relevant_group');
                   return FALSE;
               }
               if($user->group_id!= 1 && $user->warehouse_id!= $store_id)
               {
                   $this->set_error('please_select_relevant_store');
                   return FALSE;
               } 
                if($user->group_id == 5 ){if($till_ip != $user_ip ){
                   $this->set_error('You are not assigned to this till');
                   return FALSE;
                  } 
                }*/
               // -------- End add code @ Ankit---------- 
        ...
        $unsuccessfull_attempts = $this->number_login_attempts($identity);
...
//            else if (strlen($identity) > 0)
     ...
     * Added by Ajay on 
     * 23-02-2017
     * To get number of login attempts
     */
    
    public function number_login_attempts($identity){
        $this->db->select('login');
        $this->db->where('login',$identity);
        $this->db->from('login_attempts');
        $count = $this->db->count_all_results();
    ...
    public function set_session($user)
    {
        ...
        $register = $this->pos_model->registerDataByWarehouse($user->warehouse_id);
        $session_data = array(
...
END) as percent")
	...
	*/
    ...
    public function getUserLastLogin(){
        $sql = "SELECT * FROM sma_user_logins WHERE user_id=".$this->session->userdata('user_id')." and CAST(time as date)='".date('Y-m-d')."' ORDER by time DESC LIMIT 1";
    ...
    public function updateUserLogoutInfo($id){
        $this->db->where('id',$id);
        $data = array('timeout'=>date('Y-m-d H:i:s'));
        if($this->db->update('user_logins',$data)){
            return TRUE;
        }else{
            return FALSE;
        }

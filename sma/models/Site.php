...
class Site extends CI_Model {
    ...
    // Added by vivek to get the biller id of the login user
    public function getBillerId($id) {
	...
	$card_details = explode('/',$no);
       
        // edited by ajay on 19-12-2016 to include CN in credit node number
        //$q = $this->db->get_where('gift_cards', array('card_no' => $card_details[2],'year'=>$card_details[1],'biller_id'=>$card_details[0]), 1);
        $q = $this->db->get_where('gift_cards', array('card_no' => $card_details[2],'biller_id'=>$card_details[0]), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
	 ...
	 * get gift card by credit voucher number and customer id
	 */

	public function getGiftCardByCustomerID($card_no,$customer_id){
		$q = $this->db->get_where('gift_cards', array('card_no' => $card_no,'customer_id'=>$customer_id), 1);
        if ($q->num_rows() > 0) {			
			$expiry = strtotime($q->row()->expiry);			
    ...
    public function getReference($field) {
        ...
        if ($q->num_rows() > 0) {
            $ref = $q->row();
            $wid = $this->session->userdata('warehouse_id') ? $this->session->userdata('warehouse_id') : '1';
                    $this->db->select('org_alias');
                    $q = $this->db->get_where('warehouses',array('id'=>$wid),1);
                    if($q->num_rows() > 0){
                        $p = $q->row();
                    }
                    $org_alias = $p->org_alias;
            //print_r($ref);
            switch ($field) {
                case 'so':
                ...
                case 'pay':
                    ...
                    break;
                case 'pos':
                    // Update by Ankit according swatch requirment..
                   // $prefix = isset($this->Settings->sales_prefix) ? $this->Settings->sales_prefix . '/POS' : '';
                    $prefix = $org_alias;
                    break;
                case 're':
                    //$type="re";
                    $prefix = $org_alias; // Update by Chitra
...
//echo "Type >> ".$ref_no;
            if ($this->Settings->reference_format == 1) {
                ...
                if($field == "re"){
                    $ref_no .= date('Y') . "/CN/" . sprintf($ref->{$field});
                }
                else{
                    $ref_no .= date('Y') . "/" . sprintf($ref->{$field});
                }
...
//echo $ref_no;exit;
    ...
    public function syncProductQty($product_id, $warehouse_id) {
        ...
        $this->db->query("update sma_products set cart_qty = 1, cart_count = 0 where id = ".$product_id." ");
	...
	public function getBalanceQuantityOfProduct($product_id, $warehouse_id = NULL) {
		
//        $this->db->select('quantity as stock', False);
...
//        return 0;
        ...
        $this->db->where('quantity_balance IS NOT NULL', null);
        $this->db->where('product_id', $product_id);
  ...
  public function getBalanceQuantityOfProductbyname($product_id, $warehouse_id = NULL) {
...
//        }
//        return 0;
        $this->db->select('products.price, products.cost, products.tax_rate, SUM(quantity_balance) AS quantity_balance');
        if ($warehouse_id) {
            $this->db->where('warehouse_id', $warehouse_id);
        }
                ...
                foreach ($items as $item) {
                    if (isset($item['pi_overselling'])) {
                        ...
                        if ($pi = $this->site->getPurchasedItem($clause)) {
                            ...
                            $this->db->update('purchase_items', array('quantity_balance' => $quantity_balance,'psale'=>'1'), $clause);
                        } else {
                            $clause['quantity'] = 0;
                            $clause['item_tax'] = 0;
    ...
    public function getSalesExecutivesByBiller($biller_id = NULL,$group_id = NULL) {
        /*if (!empty($biller_id) && !empty($group_id)) {
            ...
            $q = $this->db->get_where('users', array('biller_id' => $biller_id,'active'=>true));
            if ($q->num_rows() > 0) {
                return $q->result_array();
            }
            return FALSE;
     ...
     * Added by Ajay
     * on 29-04-2016
     * This updates balance quantity of product
     */
	public function updateBalanceQuantity($product_id,$warehouse_id = NULL,$quantity = NULL){
		$quantity_balance = $quantity-1;
	...
	*/
	public function getDefaultBiller(){
		$q = $this->db->select('default_biller')->get('pos_settings');
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
	...
	*/
    ...
    public function deleteSuspendBills($sid){
        $q = $this->db->where(array('id'=>$sid))->get('suspended_bills');   
        if ($q->num_rows() > 0) {
            $sus_id = array();                  
            return $sus_id;
        }   

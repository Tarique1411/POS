...
<!--    <p style="text-align:center;">&copy; <?= date('Y') . " " . $Settings->site_name; ?> (v<?= $Settings->version; ?>
    ...
    <p style="text-align:center;">&copy; <?= date('Y') . " " . "MySoftWorld Software solutions Pvt. Ltd."; ?> <?php if ($_SERVER["REMOTE_ADDR"] == '127.0.0.1') {
            echo ' - Page rendered in <strong>{elapsed_time}</strong> seconds';
        } ?>
    </p>
</footer>
...
<script type="text/javascript" src="<?= $assets ?>js/creditcardtypes.js"></script>
<script type="text/javascript" src="<?= $assets ?>js/jquery.dataTables.min.js"></script>
<script type="text/javascript" src="<?= $assets ?>js/jquery.dataTables.dtFilter.min.js"></script>
<script type="text/javascript" src="<?= $assets ?>js/select2.min.js"></script>
<!--<script type="text/javascript" src="<?= $assets ?>pos/js/keyboard/jquery-ui.min.js"></script>-->
...
<script type="text/javascript" src="<?= $assets ?>pos/js/common.virtual.js"></script>
...
<script type="text/javascript" charset="UTF-8">var r_u_sure = "<?=lang('r_u_sure')?>";
    ...
    $(document).ready(function () { 
      <?php if($this->router->class!='reports'){
        ...
        var fields = $('.form-control').not('.amount,select,:hidden,:button');
        $(fields).addClass('kb-text');
        $('.input-xs').addClass('kb-text');
        //$('#' + $(this).attr('id')).not('.amount').addClass('kb-text');		
	display_keyboards();			
    <?php

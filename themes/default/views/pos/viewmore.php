...
<div id="wrapper">
    ...
    <div id="receipt-data">
        <div class="text-center">
			...
			echo "<p>" . lang("invoice_number") . ": " . $inv->id;
            echo "<p>" . lang("sales_exe_id") . ": " . $inv->sales_executive_id."<br>";
            ...
            <table class="table table-striped table-condensed">
				<thead>
					<tr>
						...
						<th>Discounted Price(if line item discount)<th>
					</tr>
				</thead>
                <tbody>
                <?php
                ...
                foreach ($rows as $row) {
                    ...
                    echo '<tr><td>#' . $r . ': &nbsp;&nbsp;</td><td>' . product_name($row->product_name) . ($row->variant ? ' (' . $row->variant . ')' : '').$booking_status.'</td>';
                    echo '<td>' . intval($row->quantity) . '';
					/*
					...
					*/
                    ...
                    echo '</td><td class="text-left">' . $this->sma->formatMoney($row->net_unit_price + $row->item_discount) . '</td><td>'.$this->sma->formatMoney($row->net_unit_price).'</td></tr>';
                    $r++;
                }
                ?>
                </tbody>
                ...
                <tr>
					...
					<th><?=$this->sma->formatMoney($total_discounted_amount)?></th>
					<!---
				...
				<tr>
					...
					<th></th>
					<th colspan="1">Discount Amount</th>
                ...
                ?>
                    ...
                    <tr>
                        ...
                        <th class="text-left" colspan="2"><?= $this->sma->formatMoney($inv->grand_total + $rounding); ?></th>
                    </tr>
                <?php } else { ?>
                    <tr>
                        <th colspan="4"><?= lang("grand_total"); ?></th>		
                        <th class="text-left" colspan="1"><?= $this->sma->formatMoney($inv->grand_total); ?></th>
                    </tr>
                <?php }
                if ($inv->paid < $inv->grand_total) { ?>
                    <tr>
                        <th colspan="4"><?= lang("paid_amount"); ?></th>					
                        <th class="text-left" colspan="2"><?= $this->sma->formatMoney($inv->paid); ?></th>
                    </tr>
                    <tr>
                        <th colspan="4"><?= lang("due_amount"); ?></th>
                        <th class="text-left" colspan="2"><?= $this->sma->formatMoney($inv->grand_total - $inv->paid); ?></th>
                    </tr>
                <?php } ?>
                </tfoot>
            </table>
            ...
            <table class="table table-striped table-condensed">
				<?php
					//echo "<pre>";print_r($allbillers);die;
					foreach($allbillers as $k=>$v){
				?>
					<tr>
						<td><?=lang('customer_service_detail')?></td>
						<td><?=$v->company?></td>
						<td><?=$v->city?></td>
						<td><?=$v->address?></td>
						<td><?=$v->phone?><td>

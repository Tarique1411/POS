            ...
            <div class="col-lg-12">
                ...
                <div class="col-md-5">
                    ...
                    <div class="form-group all">
					 ...
					 <div class="form-group all">
                        <?= lang("Product_Max_Discount_%", "max_discount") ?>
                        <?= form_input('max_discount', (isset($_POST['max_discount']) ? $_POST['max_discount'] : ($product ? $this->sma->formatDecimal($product->max_discount) : '')), 'class="form-control tip" id="max_discount" required="required"') ?>
                    </div>
				...
				<div class="col-md-12">
					<div class="form-group">
                        <input name="serialized" type="checkbox" class="checkbox" id="serialized"
                               value="on" <?= !empty($_POST['serialized']) || !empty($product->serialized) ? 'checked="checked"' : '' ?>/>
							   <label for="serialized" class="padding05"><?= lang('Serialized','serialized') ?></label>
                    </div>
					<div class="row" id="serialized-con" style="display: none;">
						 <div class="col-md-5">
                            <div class="form-group all">
                                <?= lang('Serial_Number', 'serial_number') ?>
                                <?= form_input('serial_number', (isset($_POST['serial_number']) ? $_POST['serial_number'] : ($product ? $product->serial_number : '')), 'class="form-control tip" id="serial_number"') ?>
                            </div>
                        </div>
					</div>
				</div>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        //$('#cost').removeAttr('required');
		 ...
		 <?=isset($_POST['serialized']) ? '$("#serialized").iCheck("check");': '' ?>
		  
        $('#serialized').on('ifChecked', function () {
            $('#serialized-con').slideDown();
        });
        $('#serialized').on('ifUnchecked', function () {
            $('#serialized-con').slideUp();
        });

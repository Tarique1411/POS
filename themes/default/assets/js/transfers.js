 ...
 function loadItems() {
    ...
    if (localStorage.getItem('toitems')) {
        ...
        $.each(toitems, function () {
            ...
            var inv_no = item.row.inv_no;
            var inv_date = item.row.inv_date; //alert("inv_date"+inv_date);
            var product_hsn = item.row.hsn;
            ...
            tr_html = '<td><input name="product_id[]" type="hidden" class="rid" value="' + product_id + '"><input name="product_type[]" type="hidden" class="rtype" value="' + item_type + '"><input name="product_lot[]" type="hidden" class="plot" value="' + lot_no + '"><input name="product_hsn[]" type="hidden" class="rtypes" value="' + product_hsn + '"><input name="product_code[]" type="hidden" class="rcode" value="' + item_code + '"><input name="product_name[]" type="hidden" class="rname" value="' + item_name + '"><input name="product_option[]" type="hidden" class="roption" value="' + item_option + '"><span class="sname" id="name_' + row_no + '">' + item_name + ' (' + item_code + ')'+(sel_opt != '' ? ' ('+sel_opt+')' : '')+'</span> </td>';
            ...
            tr_html += '<td class="text-left"><input class="form-control input-sm text-right" name="invno[]" type="hidden" id="invno_' + row_no + '" value="' + inv_no + '"><span class="text-right " id="invnos_' + row_no + '">' + inv_no + '</span></td>';
            tr_html += '<td class="text-left"><input class="form-control input-sm text-right" name="invdate[]" type="hidden" id="invdate_' + row_no + '" value="' + inv_date + '"><span class="text-right " id="invdates_' + row_no + '">' + inv_date + '</span></td>';
            tr_html += '<td class="text-left"><input class="form-control input-sm text-right" name="quantity_bal[]" type="hidden" id="quantity_bal' + row_no + '" value="' + formatDecimal(item_aqty) + '"><span class="text-right " id="quantity_bals_' + row_no + '">' + formatDecimal(item_aqty) + '</span></td>';
            
            tr_html += '<td><input name="quantity_balance[]" type="hidden" class="rbqty" value="' + formatDecimal(item_aqty) + '"><input class="form-control text-center rquantity" name="quantity[]" type="text" value="' + formatDecimal(item_qty) + '" data-id="' + row_no + '" data-item="' + item_id + '" id="quantity_' + row_no + '" onClick="this.select();"></td>';
            if (site.settings.tax1 == 1) {
 ...
 * @returns {Boolean}
 ...
 function add_transfer_item(item) {
    ...
    var cont = 0; var cont12 = 0;
    var fitm= item.row.code;    
      $.each(toitems, function (k,v) { 
          if(v.row.code==fitm) {
            cont12 = cont12 + 1;
           bootbox.alert("Product already added in cart");

...
$(document).ready(function() {
    ...
    $('body').on('click', '#z_xls', function(e) {
        e.preventDefault();
        $('#z_form_action').val($(this).attr('data-action'));
        $('#z_action-form').submit();
    });
    $('body').on('click', '#z_pdf', function(e) {
        e.preventDefault();
        $('#z_form_action').val($(this).attr('data-action'));        
        $('#z_action-form').submit();
    });
...
//    });
	...
	var biller = $('#biller_id').val();
        var no = generateCardNo(8,biller);
...
function generateCardNo(x,biller) {
   ...
   return biller + '/' + year + '/CN/' + n;
}
function generateLotNo(x) {
    
    if(!x) { x = 6; }
    chars = "1234567890ABCDEFGHIJ";
    no = "";
    for (var i=0; i<x; i++) {
       var rnum = Math.floor(Math.random() * chars.length);
       no += chars.substring(rnum,rnum+1);
...
function user_status(x) {
    ...
    '<a href="'+site.base_url+'auth/activate/'+ y[1] +'" data-toggle="modal" data-target="#myModal"><span class="label label-danger"><i class="fa fa-times"></i> '+lang['inactive']+'</span><a/>';
}
...
$(document).ready(function() {
    ...
    $('body').on('click', '.z_report_link td:not(:first-child :last-child)', function() {
        $('#myModal').modal({ remote: site.base_url + 'reports/view/' + $(this).parent('.z_report_link').attr('id') + '/1' });
    });
